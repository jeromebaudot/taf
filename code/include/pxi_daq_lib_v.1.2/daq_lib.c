...
FIL__TCBinFile      APP__VGRunConfFile ( (char*)"./err_TCBinFile.txt"   , 1 /* EnableErrLog */, ERR_LOG_LVL_ERRORS );
FIL__TCStreamFile   APP__VGRunDataFile ( (char*)"./err_TCStreamFile.txt", 1 /* EnableErrLog */, ERR_LOG_LVL_ERRORS, 512 /* DiskBlocSz */ );
...
SInt32 APP__FPrintRecSzChkAlign ( char* Os ) {
  ...
  msg (( MSG_OUT, "FIL__TCStreamFile_TBlocInf    = %lu ", sizeof (FIL__TCStreamFile_TBlocInf)    ));
  msg (( MSG_OUT, "FIL__TCStreamFile_TRecInfFile = %lu ", sizeof (FIL__TCStreamFile_TRecInfFile) ));
  msg (( MSG_OUT, "EFRIO__TRunCont               = %lu ", sizeof (EFRIO__TRunCont) ));
  msg (( MSG_OUT, "------------------------------------------------------------" ));
  msg (( MSG_OUT, "EFRIO__TFrame                 = %lu ", sizeof (EFRIO__TFrame) ));
  msg (( MSG_OUT, "EFRIO__TFrameHeader           = %lu ", sizeof (EFRIO__TFrameHeader) ));
  msg (( MSG_OUT, "EFRIO__TFrameData             = %lu ", sizeof (EFRIO__TFrameData) ));
  msg (( MSG_OUT, "EFRIO__TTriggerRec            = %lu ", sizeof (EFRIO__TTriggerRec) ));
...
SInt32 APP__FLoadRun ( char* RunDir, char* RunPrefix, SInt32 RunNo ) {
  ...
  //SInt32 sizeof_EFRIO__TRunCont = sizeof (EFRIO__TRunCont);
...
SInt32 APP__FAllocAcqBuffer () {
  ...
  // MI26__ZS_FFRAME_RAW_MAX_W8 replaced by ULT1__ZS_FFRAME_RAW_MAX_W8, JB+GC 2014/02/07
