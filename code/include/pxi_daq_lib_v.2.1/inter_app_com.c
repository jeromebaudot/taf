...
Date      : 10/11/2008
Rev       : 31/01/2013
          : - Add parameter "Caller" for debug pb Mi28 NI6562 DAQ
Doc date  : //2008
Author    : Gilles CLAUS
E-mail    : claus@lepsi.in2p3.fr
Labo      : LEPSI */
...
SInt32 IAC__FSockSendShortMsg ( SInt32 ContextId, UInt8* PtMsg, UInt32 MsgSz, char* Caller ) {
  ...
  err_retfail ( IAC__PRIV_FChkSenderContextId (ContextId), (ERR_OUT,"Caller=%s - Bad Id=%d", Caller, ContextId ) );
  ...
  if ( VPtSocket->Started != 1 ) {
    err_retfail ( -1, (ERR_OUT,"Caller=%s - Access [%d] not started => Can't send message", Caller, VPtSocket->Id ) );
  }
  ...
  if ( VPtSocket->ParLongMsg == 1 ) {
    err_retfail ( -1, (ERR_OUT,"Caller=%s - This port doesn't support short message ! ParLongMsg=%d", Caller, VPtSocket->ParLongMsg ) );
  }
  ...
  if ( ( MsgSz + IAC__SOCK_HEADER_TOT_SZ ) > IAC__SO_MAX_SHORT_MSG_SIZE ) {
    err_retfail ( -1, (ERR_OUT,"Caller=%s - Message too long size=%d +  IAC__SOCK_HEADER_TOT_SZ=%d > IAC__SO_MAX_SHORT_MSG_SIZE=%d", Caller, MsgSz, IAC__SOCK_HEADER_TOT_SZ, IAC__SO_MAX_SHORT_MSG_SIZE ));
  }
  
  */
  
  if ( ( MsgSz + (VPtSocket->CstHeaderSz) + (VPtSocket->CstTrailerSz) ) > IAC__SO_MAX_SHORT_MSG_SIZE ) {
    err_retfail ( -1, (ERR_OUT,"Caller=%s - Message too long size + (Header & Trailer) =%d  > IAC__SO_MAX_SHORT_MSG_SIZE=%d", Caller, MsgSz + (VPtSocket->CstHeaderSz) + (VPtSocket->CstTrailerSz), IAC__SO_MAX_SHORT_MSG_SIZE ));
  }
  ...
  for ( ViTry=0; ViTry < VPtSocket->ParRetryNb; ViTry++ ) {
    ...
    if ( PulseEvent ( VPtSocket->EvNewMsgHnd ) == TRUE ) {
      err_trace (( ERR_OUT, "Caller=%s - Event EvNewMsgHnd sent", Caller ));
    }
    ...
    err_retnull ( VPtSocket->EvAckMsgHnd, (ERR_OUT,"Caller=%s - Ack event not installed !", Caller ) );
      
    // Time out => Retry  
      
    if ( WaitForSingleObject ( VPtSocket->EvAckMsgHnd, VPtSocket->ParAckTimeOutMs ) == WAIT_TIMEOUT ) {
      err_error (( ERR_OUT, "Caller=%s - No Ack [TO=%d ms]=> Exit on time out !! ViTry=%d - Msg : %s", Caller, VPtSocket->ParAckTimeOutMs, ViTry, PtMsg ));
      continue;
    }
    ...
    if ( VPtSocket->SendStatus <= 0 ) {
      err_error (( ERR_OUT, "Caller=%s - Send error - ViTry=%d - Msg : %s", Caller, ViTry, PtMsg ));
      continue;
    }
    
    // OK message sent, Ack received => break loop
    
    err_trace (( ERR_OUT, "Caller=%s - Ack received", Caller ));
    break;
    
      
  } /* End for */
  
  // MS 10 01 2014 add printing of the msg
  /*for (ViMsgWord = 0 ; ViMsgWord < MsgSz ; ViMsgWord++){
      err_warning (( ERR_OUT, "Word[%d] = %d ",ViMsgWord, PtMsg[ViMsgWord] ));
...
SInt32 IAC__FSockSendLongMsg ( SInt32 ContextId, UInt8* PtMsg, UInt32 MsgSz, char* Caller ) {
  ...
  if ( VPtSocket->ParLongMsg == 0 ) {
    err_retfail ( -1, (ERR_OUT,"Caller=%s - This port doesn't support long message ! ParLongMsg=%d", Caller, VPtSocket->ParLongMsg ) );
  }
  ...
  if ( VPtSocket->ParMsgSz > IAC__SOCK_MAX_LONG_MSG_SZ ) {
    err_retfail ( -1, (ERR_OUT,"Caller=%s - Message too long size=%d > IAC__SOCK_MAX_LONG_MSG_SZ=%d", Caller, MsgSz, IAC__SOCK_MAX_LONG_MSG_SZ ));
  }
  ...
  err_trace (( ERR_OUT, "Caller=%s - LongMsgSz = %d", Caller, VPtSocket->ParMsgSz ));
  ...
  for ( ViTry=0; ViTry < VPtSocket->ParRetryNb; ViTry++ ) {
    ...
    if ( WaitForSingleObject ( VPtSocket->EvAckMsgHnd, VPtSocket->ParAckTimeOutMs  ) == WAIT_TIMEOUT	 ) {
      err_error (( ERR_OUT, "Caller=%s - No Ack => Exit on time out %d [ms] !!! ViTry=%d - Msg : %s", Caller, VPtSocket->ParAckTimeOutMs, ViTry, PtMsg ));
      continue;
    }
...
SInt32 IAC__PRIV_FSockSendShortMsg ( SInt32 ContextId ) {
  ...
  */
  ...
  if ( VSzRec <= 0 ) {
    VSockErrNo = IAC__SOCKET_ERRNO;
    VRet = -1;
    err_error (( ERR_OUT, "Ack reception error %d - %s", VSockErrNo, strerror (VSockErrNo) ));
  }
...
SInt32 IAC__PRIV_FSockSendLongMsg ( SInt32 ContextId ) {
  ...
  for ( ViBlock=0; ViBlock < VBlockNb; ViBlock++) {
    ...
    if ( VSzSent == SOCKET_ERROR ) {
      VSockErrNo = IAC__SOCKET_ERRNO;
      err_retfail ( -1, (ERR_OUT,"send () error %d - Block[%d] => %s", VSockErrNo, ViBlock, strerror (VSockErrNo) ));
    }
  ...
  if ( VLastBlockSz > 0 ) {
    ...
    if ( VSzSent == SOCKET_ERROR ) {
      VSockErrNo = IAC__SOCKET_ERRNO;
      err_retfail ( -1, (ERR_OUT,"send () error %d - Last block => %s", VSockErrNo, strerror (VSockErrNo) ));
    }
  ...
  if ( VMsgSingleBlock == 0 ) {
    ...
    if ( VSzSent == SOCKET_ERROR ) {
      VSockErrNo = IAC__SOCKET_ERRNO;
      err_retfail ( -1, (ERR_OUT,"send () error %d - Trailer => %s", VSockErrNo, strerror (VSockErrNo) ));
    }
  ...
  if ( VSzRec <= 0 ) {
    VSockErrNo = IAC__SOCKET_ERRNO;
    err_retfail ( -1, (ERR_OUT,"Ack receive error %d -  %s", VSockErrNo, strerror (VSockErrNo) ));
  }
...
SInt32 DEBUG___IAC__PRIV_FSockSendLongMsg ( SInt32 ContextId ) {
  ...
  for ( ViBlock=0; ViBlock < VBlockNb; ViBlock++) {
    ...
    } // End if ( VSockErrNo == WSAECONNRESET )
...
SInt32 IAC__FSockInstallSenderPort ( char* DestHost, UInt32 DestPort, SInt8 LongMsg, UInt32 AckTimeOutMs, UInt8 RetryNb, IAC__TFSockFuncCheckSum FuncCheckSum ) {
  ...
  if ( VPtSocket->Socket == INVALID_SOCKET ) {
    VSockErrNo = IAC__SOCKET_ERRNO;
    err_retfail ( -1, (ERR_OUT,"socket () error %d => %s", VSockErrNo, strerror (VSockErrNo) ));
  }
  ...
  if ( bind( VPtSocket->Socket, (struct sockaddr *)&(VPtSocket->LocAddr), sizeof(VPtSocket->LocAddr)) == SOCKET_ERROR ) {
    VSockErrNo = IAC__SOCKET_ERRNO;
    err_retfail ( -1, (ERR_OUT,"bind () error %d => %s", VSockErrNo, strerror (VSockErrNo) ));
  }
...
SInt32 IAC__FSockRebootSenderPort ( SInt32 ContextId ) {
  ...
  while ( connect( VPtSocket->Socket, &(VPtSocket->DestAddr), sizeof(VPtSocket->DestAddr) ) == SOCKET_ERROR ) {
    VSockErrNo = IAC__SOCKET_ERRNO;
    err_error (( ERR_OUT, "Connect () error %d => %s WILL RETRY in 100 ms", VSockErrNo, strerror (VSockErrNo) ));
    Sleep ( 100 );
  }
...
SInt32 IAC__FSockRebootReceiverPort ( SInt32 ContextId ) {
  ...
  if ( listen( VPtSock->LSocket, 2) == SOCKET_ERROR ) {
    VSockErrNo = IAC__SOCKET_ERRNO;
    err_retfail ( -1, (ERR_OUT,"listen () error %d => %s", VSockErrNo, strerror (VSockErrNo) ));
  }
  ...
  if( VPtSock->ASocket == INVALID_SOCKET )  {
    VSockErrNo = IAC__SOCKET_ERRNO;
    err_retfail ( -1, (ERR_OUT,"accept () error %d => %s", VSockErrNo, strerror (VSockErrNo) ));
  }
...
SInt32 IAC__PRIV_FSockLookForShortRecMsgOnPortAndProcessIt ( UInt32 RecPortContextId ) {
  ...
  while (1) {
    ...
    if( VAckSentSz == SOCKET_ERROR ) {
      VSockErrno = IAC__SOCKET_ERRNO;
      err_retfail ( -1, (ERR_OUT,"send () Ack error %d => %s", VSockErrno, strerror (VSockErrno) ));
    }
...
/*  BEFORE 14/01/09
    ...
    if( VRecBlocSz <= 0 ) {
      ...
      if ( (VRecBlocSz == 0) ) {
        ...
        VSockErrNo = IAC__SOCKET_ERRNO;
        err_warning (( ERR_OUT, "recv () error %d => %s", VSockErrNo, strerror (VSockErrNo) ));
        continue;
      }      
...
DWORD WINAPI IAC__PRIV_FSockSenderThreadFunc ( LPVOID lpParam ) {
  ...
  err_error (( ERR_OUT, "----------------------------------------------------" ));
  err_error (( ERR_OUT, "Thread execution of sender port [%d] started", VPtSock ->Id   ));
  err_error (( ERR_OUT, "Dest host = %s", VPtSock ->ParDestHost     ));
  err_error (( ERR_OUT, "Dest port = %d", VPtSock ->ParDestPort     ));
  err_error (( ERR_OUT, "Ack TO    = %d", VPtSock ->ParAckTimeOutMs ));
  err_error (( ERR_OUT, "----------------------------------------------------" ));
  ...
  while ( connect( VPtSock->Socket, &(VPtSock->DestAddr), sizeof(VPtSock->DestAddr) ) == SOCKET_ERROR ) {
    VSockErrNo = IAC__SOCKET_ERRNO;
    err_warning (( ERR_OUT, "Connect () error %d => %s WILL RETRY in 100 ms", VSockErrNo, strerror (VSockErrNo) ));
    Sleep ( 100 );
  }
...
DWORD WINAPI IAC__PRIV_FSockReceiverThreadFunc ( LPVOID lpParam ) {
  ...
  err_error (( ERR_OUT, "----------------------------------------------------" ));
  err_error (( ERR_OUT, "Thread execution of receiver port [%d] started", VPtSock->Id   ));
  err_error (( ERR_OUT, "----------------------------------------------------" ));
  ...
  while (1) {
    ...
    if ( VPtSock->DbgDisableReception == 1 ) {
      err_error (( ERR_OUT, "Reception disabled !" ));
      continue;
    }
