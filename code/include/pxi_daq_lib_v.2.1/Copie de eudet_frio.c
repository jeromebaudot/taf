...
SInt32 EFRIO__FConfRun ( SInt8 Mi26Nb, SInt32 RunNo, SInt32 TotEvNb, SInt32 EvNbPerFile, SInt32 FrameNbPerAcq, SInt8 DataTransferMode, SInt8 TrigMode, char* DestDir, char* FileNamePrefix, SInt8 SaveToDisk, SInt8 SendOnEth, SInt8 SendOnEthPCent, char* JtagFileName ) {
  ...
  VRet = EFRIO__FConfRunExt ( ASIC__MI26, Mi26Nb, RunNo, TotEvNb, EvNbPerFile, FrameNbPerAcq, DataTransferMode, TrigMode, DestDir, FileNamePrefix, SaveToDisk, SendOnEth, SendOnEthPCent, JtagFileName );
  ...
  */
  ...
  for ( ViFrameWithTrig=0; ViFrameWithTrig < VFrameWithTrigCnt; ViFrameWithTrig++ ) {
    ...
    */
    ...
    VDataLengthW16Max = MATH_FUInt16Max ( VADataLengthW16, Mi26Nb-1 );
    ...
    */
    ...
    // => Set maximum data size variable and data size fields to MAXIMUM possible value
    // because for extra trigger acquisition, we must process the full frame
    
    else {
      
      for ( ViMi26=0; ViMi26 < Mi26Nb /* 8 */; ViMi26++ ) {
        VADataLengthW16[ViMi26] = 1140;
        VADataLengthW8[ViMi26]  = VADataLengthW16[ViMi26] * 2;
        VADataLengthW32[ViMi26] = VADataLengthW16[ViMi26] / 2;
      }
    ...
    */
    ...
    // 14/06/13 : Force to test only 6 Mi26 because the last channel is not a Mi26 but the extra trigger info
