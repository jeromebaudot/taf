...
/*******************************************************************************
Prototype : SInt32 FSBB0_FDecodeString(char * Buffer, SInt16 DataType)
...
SInt32 FSBB0_FDecodeString (char * Buffer, SInt16 DataType){
	SInt32 VReturn;
	SInt32 VIndex;
	SInt8  VNumberIndex;
	SInt8  VMajorNumberDone;
	SInt32 VCurrentNumber;
	SInt8 VJunkDone;
	...
	switch (DataType){
		case 0:{ // Configuration
			while((Buffer[VIndex]!=0xa)&&(VIndex < strlen(Buffer))){
				if ((Buffer[VIndex] >=0x30) &&(Buffer[VIndex] <= 0x39)){
					VReturn = Buffer[VIndex] - 0x30;
					return( VReturn);
				}
				VIndex++;
		...
		case 1:{  // Pattern File Number
			while((Buffer[VIndex]!=0xa)&&(VIndex < strlen(Buffer))){
				if (Buffer[VIndex] == 0x3A){
					VJunkDone = 1;
				}
				if ((VJunkDone != 0 )){
					if ((Buffer[VIndex] >=0x30) &&(Buffer[VIndex] <= 0x39)){
							VCurrentNumber = (VCurrentNumber * 10) + (Buffer[VIndex] - 0x30);
							VNumberIndex++;
					}
					else{
						if ((VNumberIndex != 0)){
							VMajorNumberDone = 1;
							VReturn = VCurrentNumber << 16;
							VCurrentNumber = 0;
							VNumberIndex = 0;
						}
					}
				...
				VIndex++;
				if ((Buffer[VIndex]==0xa)||(VIndex >= strlen(Buffer))){
					if (VMajorNumberDone == 0){
						VReturn = VCurrentNumber << 16;
					}
					else{
						VReturn = VReturn + VCurrentNumber;
					}
					return (VReturn);
				}				
			}/* end while  not end of line*/		
			...
			while((Buffer[VIndex]!=0xa)&&(VIndex < strlen(Buffer))){
				switch (Buffer[VIndex]){
					...
					case 0x39:{
						...
						VCurrentNumber = (VCurrentNumber << 4) + (Buffer[VIndex] - 0x30);
						VNumberIndex++;
					...
					case 0x46:{
						...
						VCurrentNumber = (VCurrentNumber << 4) + (Buffer[VIndex] - 0x37);
						VNumberIndex++;
					...
					case 0x66:{
						...
						VCurrentNumber = (VCurrentNumber << 4) + (Buffer[VIndex] - 0x57);
						VNumberIndex++;
		...
		case 4:{ // Zone
			while((Buffer[VIndex]!=0xa)&&(VIndex < strlen(Buffer))){
				...
				if ((VJunkDone != 0 )&&((Buffer[VIndex] >=0x30) &&(Buffer[VIndex] <= 0x39))){
						VCurrentNumber = (VCurrentNumber * 10) + (Buffer[VIndex] - 0x30);
						VNumberIndex++;
				}
				VIndex++;
...
}/* end FSBB0_FDecodeString  */

/*******************************************************************************
Prototype : SInt32 FSBB0_FReadConfigFile(char * FilePath)
...
/******************************************************************************/
 ...
 FSBB0__TTestPattern FSBB0_FReadConfigFile(char * FilePath, SInt8 PrintLevel)
 {
	FILE * pSourceFile;
	char   VBuffer[128];
	SInt32 VDataW32;
	SInt8 ViLineIndex;
	SInt8 ViColIndex;
	FSBB0__TTestPattern  VTestPattern;

  // Open source file

	err_error (( ERR_OUT, "FilePath : %s",FilePath ));


  pSourceFile = fopen ( FilePath , "rb");
	if(pSourceFile==NULL) {
		err_error (( ERR_OUT, "Error: can't open config file." ));
		printf("Error: can't open source file.\n");
		VTestPattern.ConfigIndex = -1;
		return VTestPattern;
	} 
	
	// read first line : Configuration:X
	fgets(VBuffer,40, pSourceFile);
	VTestPattern.ConfigIndex = FSBB0_FDecodeString (VBuffer, 0);
	if ((PrintLevel == 6)||(PrintLevel == 7)){		
		err_error (( ERR_OUT, "VTestPattern.ConfigIndex : %d",VTestPattern.ConfigIndex ));
	}

	// read two comments lines
	fgets(VBuffer,40, pSourceFile);
	fgets(VBuffer,40, pSourceFile);
	// read the file pattern number
	fgets(VBuffer,100, pSourceFile);
	VDataW32 = FSBB0_FDecodeString (VBuffer, 1);
	VTestPattern.FileNumberMajor  = (VDataW32 & 0xffff0000)>>16;
	VTestPattern.FileNumberMinor  = VDataW32 & 0xffff;
	if ((PrintLevel == 6)||(PrintLevel == 7)){		
		err_error (( ERR_OUT, "VTestPattern.FileNumberMajor:%d ,VTestPattern.FileNumberMinor:%d",VTestPattern.FileNumberMajor,VTestPattern.FileNumberMinor ));
	}
	// read the 8 lines pattern registers
	for (ViLineIndex = 0; ViLineIndex < 8 ; ViLineIndex++){
		// read the line number
		fgets(VBuffer,40, pSourceFile);
		// read the pattern registers value
		for (ViColIndex = 0 ; ViColIndex < 13 ; ViColIndex++){
			fgets(VBuffer,40, pSourceFile);
			VTestPattern.Pattern[ViLineIndex][ViColIndex] = FSBB0_FDecodeString (VBuffer, 3);
		}/* End for ViColIndex */
	} /* End for ViLineIndex */
	// read one comments lines
	fgets(VBuffer,40, pSourceFile);
	// read the zone definition
	fgets(VBuffer,40, pSourceFile);
	VTestPattern.Zone00 = FSBB0_FDecodeString (VBuffer, 4);	
	if ((PrintLevel == 6)||(PrintLevel == 7)){		
		err_error (( ERR_OUT, "Zone00 : %d",VTestPattern.Zone00 ));
	}
	fgets(VBuffer,40, pSourceFile);
	VTestPattern.Zone01 = FSBB0_FDecodeString (VBuffer, 4);	
	if ((PrintLevel == 6)||(PrintLevel == 7)){		
		err_error (( ERR_OUT, "Zone01 : %d",VTestPattern.Zone01 ));
	}
	fgets(VBuffer,40, pSourceFile);
	VTestPattern.Zone10 = FSBB0_FDecodeString (VBuffer, 4);	
	if ((PrintLevel == 6)||(PrintLevel == 7)){		
		err_error (( ERR_OUT, "Zone10 : %d",VTestPattern.Zone10 ));
	}
	fgets(VBuffer,40, pSourceFile);
	VTestPattern.Zone11 = FSBB0_FDecodeString (VBuffer, 4);	
	if ((PrintLevel == 6)||(PrintLevel == 7)){		
		err_error (( ERR_OUT, "Zone11 : %d",VTestPattern.Zone11 ));
	}
...
char * FSBB0_FGetConfigName(SInt16 ConfigNr)
 {
	FILE * pSourceFile;
	char  VBuffer[200];
	char   VReturnBuffer[50];
	SInt8 ViLineIndex;
  ...
  pSourceFile = fopen ( ".\\GD_Files\\description_test_pat.txt" , "rb");
	if(pSourceFile==NULL) {
		err_error (( ERR_OUT, "Error: can't open config file." ));
		printf("Error: can't open source file.\n");
		//VTestPattern.ConfigIndex = -1;
	...
	// read first line : comments
	fgets(VBuffer,80, pSourceFile);
	// Ignore ConfigNr lines
	if (ConfigNr > 0){
		for (ViLineIndex = 0 ; ViLineIndex < ConfigNr ; ViLineIndex++){
			fgets(VBuffer,200, pSourceFile);
		}
	}
	// get the wanted line
	fgets(VBuffer,200, pSourceFile);
	
	strncpy ( VReturnBuffer , VBuffer , 50);

	fclose(pSourceFile);

	return (VReturnBuffer);

}  /*  end FSBB0_FReadConfigFile  */


/*******************************************************************************
Prototype : SInt32 FSBB0_FReadResultBitFile(char * FilePath)

Goal      :
Inputs    :
Ouputs    : returns the number of bit at 1 of the matrix
Globals   :
Remark    : 
Level     :
Date      : 18/06/2014
...
/******************************************************************************/
 ...
 FSBB0__TMatDiscriBit FSBB0_FReadResultBitFile(char * FilePath, SInt8 PrintLevel)
 {
	SInt32 	VResult;
	FILE * pSourceFile;
	char   VBuffer[420];
	...
	SInt16 ViLineIndex;
	SInt16 ViColIndex;
	FSBB0__TMatDiscriBit VMatResult;
  ...
  // Open source file
	pSourceFile = fopen ( FilePath , "rb");
	if(pSourceFile==NULL) {
		err_error (( ERR_OUT, "Error: can't open result bit file." ));
		printf("Error: can't open source file.\n");
		VMatResult.AALineCol[0][0] = -1;
		return VMatResult;
	} 
	err_error (( ERR_OUT, "Filename: %s",FilePath ));

	VResult = 0;
	// read  first line : comments
	fgets(VBuffer,80, pSourceFile);
	// read second line : image nr
	fgets(VBuffer,40, pSourceFile);
	VDataW32 = FSBB0_FDecodeString (VBuffer, 1);
	FileNumberMajor  = (VDataW32 & 0xffff0000)>>16;
	FileNumberMinor  = VDataW32 & 0xffff;
	if ((PrintLevel == 6)||(PrintLevel == 7)){
		err_error (( ERR_OUT, "FileNumberMajor:%d ,FileNumberMinor:%d",FileNumberMajor,FileNumberMinor ));
	}
	// read third line : comments
	fgets(VBuffer,140, pSourceFile);
	// read the 416 lines of 416 bits
	// 26/06/2014 - MS : line number were starting from 0 to 416 abd it should be from 416 to 0
	for (ViLineIndex = (FSBB0__MAT_DISCRI_USEFUL_LINES_NB - 1); ViLineIndex >= 0 ; ViLineIndex--){
		// read the line of 416 bits
		fgets(VBuffer,418, pSourceFile);
		for (ViColIndex = 0 ; ViColIndex < FSBB0__MAT_DISCRI_COL_NB ; ViColIndex++){
			VReadChar = VBuffer[ViColIndex];
			if (VReadChar == 0x31){
				VMatResult.AALineCol[ViLineIndex][(FSBB0__MAT_DISCRI_COL_NB - 1) - ViColIndex] = 1;
				VResult++;
				if ((PrintLevel == 6)||(PrintLevel == 7)){
					err_error (( ERR_OUT, "Hit Line :%d, col:%d",ViLineIndex,(FSBB0__MAT_DISCRI_COL_NB - 1) - ViColIndex ));
				}
			}
			else{
				VMatResult.AALineCol[ViLineIndex][(FSBB0__MAT_DISCRI_COL_NB - 1) - ViColIndex] = 0;
			}
		}/* End for ViColIndex */
...
//#endif
  ...
  msg (( MSG_OUT, "UsefulDataLengthW30   = %4d [D]", Pt->UsefulDataLengthW30 ));
...
SInt32 FSBB0__FDiscriCopySubMatBit ( FSBB0__TMatDiscriBit* PtSrc, FSBB0__TSubMatDiscriBit* PtDest, SInt8 SubMatId ) {
  ...
  switch (SubMatId){
	case 0:{
	    ...
	    break; }
	case 1:{
		VColOffset = FSBB0__REG_DISCRI_BIT_SZ / FSBB0__SUB_MAT_NB;
	    break;  }
	default :{
  ...
  for ( ViLine=0; ViLine < FSBB0__MAT_DISCRI_USEFUL_LINES_NB; ViLine++ ) {
    ...
    for ( ViCol=0; ViCol <= (FSBB0__REG_DISCRI_BIT_SZ/ FSBB0__SUB_MAT_NB); ViCol++) {
      
      PtDest->AALineCol[ViLine ][ViCol] = PtSrc->AALineCol[ViLine][ViCol + VColOffset];
  ...
  for ( ViLine=0; ViLine < LineNbToCompare; ViLine++ ) {
    ...
    for ( ViCol=0; ViCol < FSBB0__REG_DISCRI_BIT_SZ /* 580 for Test LP003 */; ViCol++ ) {
      ...
      if ( VPixState != VRefPixState ) {
        ++VErrCnt;
		// 24 06 2014 - MS commented the message to make the log file easier to read
...
SInt32 FSBB0__FPrintZSRunCnf ( FSBB0__TZSRunCnf* PtSrc ) {
  ...
  msg (( MSG_OUT, "HwTrigPar[DPXI__HW_TRIG_PAR__OFFSET] %6d", PtSrc->HwTrigPar[0/*DPXI__HW_TRIG_PAR__OFFSET*/] ));
  msg (( MSG_OUT, "HwTrigPar[DPXI__HW_TRIG_PAR__WINDOW] %6d", PtSrc->HwTrigPar[1/*DPXI__HW_TRIG_PAR__WINDOW*/] ));
  msg (( MSG_OUT, "================================================" ));
