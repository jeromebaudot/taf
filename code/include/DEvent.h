...
class DEventHeader  {
 ...
 public:
  ...
  Int_t          EListOfTriggers[MaxNumberOf];    // list of triggers JB 2010/07/07
  Int_t          ENumberOfFrames;
  Int_t          EListOfFrames[MaxNumberOf];      // list of frames JB 2010/07/07
  Int_t          ENumberOfTimestamps;
  Int_t          EListOfTimestamps[MaxNumberOf];  // list of timestamps JB 2010/07/07
...
class DEvent : public TObject {
 public:
  ...
  Int_t            fEvt;
  Int_t          fAHitsN;       // number of cluster hits in data array
  TClonesArray    *fAHits;        // pointer to hits 
  Int_t          fT1PlanesN;    // number of planes in transparent data array
  TClonesArray    *fT1Planes;     // transparent data of detector planes

  Int_t          fAPlanesN;     // number of authentic planes in array
...
class DAuthenticHit : public TObject {
 ...
 public:
  ...
  Float_t        Hv;
  Float_t        Hresu;          // spatial resolution of hit
  Float_t        Hresv;
  
  Int_t          HTS;            // time-stamp of hit
  ...
  Short_t        HsN;            // strips in cluster -> changed to seed SNR, JB 2013/11/08
  ...
  Float_t        Hq7;
  Float_t        Hq8;
  Float_t        HqM[400];     // all  pixels in MIMOSA cluster
...
class DTransparentPlane : public TObject {
 ...
 public:
  DTransparentPlane(){;}
  DTransparentPlane(DPlane& aPlane, DTrack& aTrack, DTracker &aTracker);
  DTransparentPlane(DPlane& aPlane, DTrack& aTrack, DHit& aHit, Bool_t hitAssociated, DTracker &aTracker);
  ...
  Float_t        TvertexU;       // LC 2012/12/17. vertex position.
  Float_t        TvertexV;
  Float_t        TvertexW;
  ...
  Short_t        Thk;             // // authentic hit number in the plane, <0 if hit associated to the track 
