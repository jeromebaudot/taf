...
class DAlign : public TObject {
 private:
  ...
  TF1*         f1legendre7U;                // Define a Legendre polynomial of the order 7th, BB 21/05/2014
  TF1*         f1legendre7V;                // Define a Legendre polynomial of the order 7th, BB 21/05/2014
  TH1F*        fhAlignDU;
  TH1F*        fhAlignDV;
  TH2F*        fhAlignDVDU;
  TH2F*        fhAlignPosUV;
 ...
 public:
  ...
  void         CreateDisplayDeformation( Double_t Umin, Double_t Umax, Double_t Vmin, Double_t Vmax);   // BB 2014/05/20
  TCanvas*     GetDisplay() { return fAlignCanvas; }
  TCanvas*     GetDisplayDeformation() { return fpfxAlignCanvas; }  // BB 2014/05/20
  TCanvas*     GetDisplayDeformationFitted() { return fpfxAlignCanvas; }  // BB 2014/05/20
  void         SaveDisplay( const Char_t *fileName); // JB 2014/05/06
  void         SaveDisplayDeformation( const Char_t *fileName);                      // BB 2014/05/06
  ...
  void         Display();
  void         DisplayDeformation();                                // BB 2014/05/20
  void         DisplayDeformationFitted();                          // BB 2014/05/21
  void         Display(Int_t fRunNumber); // LC 2012/09/20.
  void         DisplayDeformation(Int_t fRunNumber);                // BB 2014/05/20
  void         DisplayDeformationFitted(Int_t fRunNumber);          // BB 2014/05/21
  ...
  void         FitDeformation(Double_t (*CoeffLegendreU), Double_t (*CoeffLegendreV), Float_t minU=0., Float_t maxU=0., Float_t minV=0., Float_t maxV=0.);                 // BB, 21/05/2014
  ...
  TF1*         GetFitDeformationU()          { return f1legendre7U; }
  TF1*         GetFitDeformationV()          { return f1legendre7V; }
