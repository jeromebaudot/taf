...
//      bar3->AddButton("RAW CHANNELS", "gTAF->GetRaw()->DisplayRawData(1.)", "Display raw channels per plane and event");
      ...
      bar3->AddButton("CUMULATE RAW CHANNELS 2D", "gTAF->GetRaw()->DisplayCumulatedRawData2D(200000)", "Display raw data per plane cumulated over 500 events in 2D");
      ...
      bar3->AddButton("CUMULATE HITS 2D", "gTAF->GetRaw()->DisplayCumulatedHits2D(200000)", "Display hits per plane cumulated over 500 events in 2D");
      ...
      if( strstr(goal, "sitrineo") || strstr(goal, "Sitrineo") ) {
        ...
        bar3->AddButton("SITRINEO/CUMUL", "gTAF->GetRaw()->SitrineoCumul(200000,2)", "Sitrineo analysis cumulated over X events");
...
/*
  // Cumulate Hits in X
  // Separately
  TCanvas *ccumulhitx44;
  g = gROOT->FindObject("ccumulhitx44") ;
  if (g) {
    ccumulhitx44 = (TCanvas*)g;
  }
  else {
    ccumulhitx44 = new TCanvas("ccumulhitx44", "Cumulate Hits in X per plane", 5, 5,800,700);
  }
  ccumulhitx44->Clear();
  ccumulhitx44->UseCurrentStyle();
  label->DrawPaveLabel(0.3,0.97,0.7,0.9999,canvasTitle);
  TPad *padx44 = new TPad("padx44","",0.,0.,1.,0.965);
  padx44->Draw();
  padx44->Divide( (Int_t)ceil(nPlanes/2.), (nPlanes>1)?2:1);
  ...
  Double_t hmeanx[3] = {hHitProjX[0]->GetMean(), hHitProjX[1]->GetMean(), hHitProjX[2]->GetMean()};
  Double_t hrmsx[3] = {hHitProjX[0]->GetRMS(), hHitProjX[1]->GetRMS(), hHitProjX[2]->GetRMS()};
  ...
  TCanvas *ccumulhitx41;
  g = gROOT->FindObject("ccumulhitx41") ;
  if (g) {
    ccumulhitx41 = (TCanvas*)g;
  }
  else {
    ccumulhitx41 = new TCanvas("ccumulhitx41", "Cumulate Hits in X superimposed", 5, 5,800,700);
  }
  ccumulhitx41->Clear();
  ccumulhitx41->UseCurrentStyle();
  label->DrawPaveLabel(0.3,0.97,0.7,0.9999,canvasTitle);
  TPad *padx41 = new TPad("padx41","",0.,0.,1.,0.965);
  padx41->Draw();
  hHitProjX[0]->Fit(fgaus1x, "R");
  hHitProjX[1]->Fit(fgaus2x, "R");
  hHitProjX[2]->Fit(fgaus3x, "R");
  
  TLegend *lx = new TLegend(0.55,0.65,0.90,0.90, "Difference of means");
  lx->SetFillStyle(0);
  lx->SetBorderSize(1);
  lx->SetTextSize(.04);
  ...
  // Cumulate Hits in Y
  // Separately
  TCanvas *ccumulhity44;
  g = gROOT->FindObject("ccumulhity44") ;
  if (g) {
    ccumulhity44 = (TCanvas*)g;
  }
  else {
    ccumulhity44 = new TCanvas("ccumulhity44", "Cumulate Hits in Y per plane", 5, 5,800,700);
  }
  ccumulhity44->Clear();
  ccumulhity44->UseCurrentStyle();
  label->DrawPaveLabel(0.3,0.97,0.7,0.9999,canvasTitle);
  TPad *pady44 = new TPad("pady44","",0.,0.,1.,0.965);
  pady44->Draw();
  pady44->Divide( (Int_t)ceil(nPlanes/2.), (nPlanes>1)?2:1);
  ...
  // Gaussian functions to fit the histos
  Double_t hmeany[3] = {hHitProjY[0]->GetMean(), hHitProjY[1]->GetMean(), hHitProjY[2]->GetMean()};
  Double_t hrmsy[3] = {hHitProjY[0]->GetRMS(), hHitProjY[1]->GetRMS(), hHitProjY[2]->GetRMS()};
  ...
  TCanvas *ccumulhity41;
  g = gROOT->FindObject("ccumulhity41") ;
  if (g) {
    ccumulhity41 = (TCanvas*)g;
  }
  else {
    ccumulhity41 = new TCanvas("ccumulhity41", "Cumulate Hits in Y superimposed", 5, 5,800,700);
  }
  ccumulhity41->Clear();
  ccumulhity41->UseCurrentStyle();
  label->DrawPaveLabel(0.3,0.97,0.7,0.9999,canvasTitle);
  TPad *pady41 = new TPad("pady41","",0.,0.,1.,0.965);
  pady41->Draw();
  hHitProjY[0]->Fit(fgaus1y, "R");
  hHitProjY[1]->Fit(fgaus2y, "R");
  hHitProjY[2]->Fit(fgaus3y, "R");

  TLegend *ly = new TLegend(0.55,0.65,0.90,0.90, "Difference of means");
  ly->SetFillStyle(0);
  ly->SetBorderSize(1);
  ly->SetTextSize(.04);
...
void MRaw::SitrineoCumul( Int_t nEvents, Int_t lastPlaneOfFirstTracker, Double_t maxX1, Double_t maxY1, Double_t maxSlopeX1, Double_t maxSlopeY1, Double_t maxDX2, Double_t maxDY2, Double_t maxSlopeX2, Double_t maxSlopeY2)
{
  ...
  TH1F *hTrackPairChi2X = new TH1F( "hTrackPairChi2X", "Track #chi2 from X-fit; #chi2", 100, 0, 100);
  TH1F *hTrackPairChi2Y = new TH1F( "hTrackPairChi2Y", "Track #chi2 from Y-fit; #chi2", 100, 0, 100);
  ...
  TH1F *hTrackPairFitX0 = new TH1F( "hTrackPairFitX0", "Track X position at origin; X [um]", 200, -10000, 10000);
  TH1F *hTrackPairFitY0 = new TH1F( "hTrackPairFitY0", "Track Y position at origin; X [um]", 200, -10000, 10000);
  ...
  for( Int_t iEvt=0; iEvt < nEvents; iEvt++) {
    ...
    for( std::vector<sitritrack_t>::iterator atrack = tracklist->begin(); atrack !=tracklist->end(); ++atrack ) { // loop on tracks
      ...
      hTrackPairFitX0->Fill( (*atrack).fit_origin_X);
      hTrackPairFitY0->Fill( (*atrack).fit_origin_Y);
  ...
  if (g) {
    ...
    ccumultrack1 = new TCanvas("ccumultrack1", "Track fit properties", 100, 100,1100,700);
  }
  ccumultrack1->Clear();
  ...
  pad5->cd(6);
  hTrackPairFitX0->Draw();
  pad5->cd(7);
  hTrackPairFitY0->Draw();
  ccumultrack1->Update();
  ...
  hTrackPairFitX0->Write();
  hTrackPairFitY0->Write();
...
Int_t MRaw::SitrineoAnalysisFromHits( Int_t lastPlaneOfFirstTracker, vector<sitritrack_t> *tracklist, Double_t minX1, Double_t maxX1, Double_t minY1, Double_t maxY1, Double_t maxSlope1)
{
  ...
  if(fDebugRaw>1) printf( " Loop on 2nd tracker with (pl3) %d and (pl4) %d hits\n", Plane1->GetHitsN(), Plane2->GetHitsN());
  ...
  if( firstTrackerPairs.size() != 0 && secondTrackerPairs.size() != 0 ){ // test there is vectors
	...
	if (atrack.fit_chi2X<10. &&/* atrack.fit_slope_X < 0.2 && atrack.fit_slope_Y < 0.2 &&*/ atrack.fit_momentum<9. && /*sqrt(fabs( pow(ftrackmodelX->Eval(-5000),2) + pow(ftrackmodelY->Eval(-5000),2) )) < 7000 && sqrt(fabs( pow(ftrackmodelX->Eval(0),2) + pow(ftrackmodelY->Eval(0),2) )) < 7000 && sqrt(fabs( pow(ftrackmodelX->Eval(5000),2) + pow(ftrackmodelY->Eval(5000),2) )) < 7000 &&*/ fabs( ftrackmodelX->Eval(-5000) )<5000 && fabs( ftrackmodelX->Eval(0) )<5000 &&fabs( ftrackmodelX->Eval(5000) )<5000 && fabs( ftrackmodelY->Eval(-5000) )<5000 && fabs( ftrackmodelY->Eval(0) )<5000 && fabs( ftrackmodelY->Eval(5000) )<5000 && atrack.fit_chi2Y<50.){
      ...
      // Remove unwanted tracks from tracklist if need be
	    if(atrack.bckup_flag==1){
	     	  for (int i = tracklist->size()-1; i>=0; i--){
		          if((*tracklist)[i].bckup_flag==0){
			        tracklist->erase(tracklist->begin()+i);
			        NbRmTracks++;
			        cout << "New track survives. Removed existing track from tracklist. NbRmTracks = " << NbRmTracks << endl;
		          }
	        }
      	  ...
      	  for (int i=tracklist->size()-1; i>=0;i--){
		      (*tracklist)[i].bckup_flag=1;
		      NbRmTracks++;
		      cout << "New track dies. Did not add track to tracklist. NbRmTracks = " << NbRmTracks << endl;
	        }      
      }
#endif
	    //tracklist->push_back(atrack);
  ...
  if(fDebugRaw) printf( "SitrineoAnalysisFromHits: ending lastPlane1stTracker %d, nPairs=%d\n", lastPlaneOfFirstTracker, (int)(tracklist->size()));
...
void MRaw::SitrineoAlign( Int_t nEvents, Bool_t order)
{
  ...
  Double_t theta_ms_rad[nPlanes]; // Devation due to multiple scattering in rad
  Double_t theta_ms_um[nPlanes]; // Deviation due to multiple scattering in um
  Double_t z_dist_um[nPlanes] = {0, 1900, 2500, 2000}; // Measured distance between planes for 4 planes (w/ plane 1 as reference)
  ...
  // Determine and draw the expected spread due to diffusion and multiple scattering on the hitmap

  if (order) { // Draw ellipse of expected opening for regular ordering
    for ( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) { // begin loop on planes
      tPlane = tTracker->GetPlane(iPlane);
      theta_ms_rad[iPlane-1] = fTool.scatteringAngle("electron", 1e-3, "silicon", 50*(iPlane-1), 0);

      // Compute the deviation due to multiple scattering in um at each plane
      if (iPlane==1) {
	      theta_ms_rad[iPlane-1] = 0;
	      theta_ms_um[iPlane-1] = 0.;
      } else if (iPlane==2) {
	      theta_ms_um[iPlane-1]=theta_ms_rad[iPlane-1]*z_dist_um[iPlane-1];
      } else {
        theta_ms_um[iPlane-1]=theta_ms_um[iPlane-2]*(1 + z_dist_um[iPlane-1]*theta_ms_rad[iPlane-1]/theta_ms_um[iPlane-2]);
      }

      Ellipse_pos_x[iPlane-1] = fgaus1x->GetParameter(1);
      Ellipse_pos_y[iPlane-1] = fgaus1y->GetParameter(1);
      Ellipse_sigma_x[iPlane-1] = sqrt(pow(fgaus1x->GetParameter(2), 2) + pow(theta_ms_um[iPlane-1], 2));
      Ellipse_sigma_y[iPlane-1] = sqrt(pow(fgaus1y->GetParameter(2), 2) + pow(theta_ms_um[iPlane-1], 2));
      ...
      PlaneEllipse[iPlane-1]->SetLineColor(iPlane);
    ...
    ccumulhit->Update();
  } else { // for reverse ordering
      for ( Int_t iPlane=nPlanes; iPlane>=1; iPlane--) { // begin loop on planes
        tPlane = tTracker->GetPlane(iPlane);
        theta_ms_rad[iPlane-1]=fTool.scatteringAngle("electron", 1e-3, "silicon", 50*(nPlanes-iPlane), 0);
        z_dist_um[0]=1900;
        z_dist_um[1]=2500;
        z_dist_um[2]=2000;
        z_dist_um[3]=0;
        ...
        if (iPlane==nPlanes) {
	      ...
	      theta_ms_um[iPlane-1]=theta_ms_rad[iPlane-1]*z_dist_um[iPlane-1];
              ...
              theta_ms_um[iPlane-1]=theta_ms_um[iPlane]*(1 + z_dist_um[iPlane-1]*theta_ms_rad[iPlane-1]/theta_ms_um[iPlane]);
        }

        Ellipse_pos_x[iPlane-1] = fgaus4x->GetParameter(1);
        Ellipse_pos_y[iPlane-1] = fgaus4y->GetParameter(1);
        Ellipse_sigma_x[iPlane-1] = sqrt(pow(fgaus4x->GetParameter(2), 2) + pow(theta_ms_um[iPlane-1], 2));
        Ellipse_sigma_y[iPlane-1] = sqrt(pow(fgaus4y->GetParameter(2), 2) + pow(theta_ms_um[iPlane-1], 2));
  ...
  if (order) {
    ...
    // In X
    Double_t sigmadiff_x[3] = {fgaus1x->GetParameter(2), sqrt(pow(fgaus2x->GetParameter(2), 2) - pow(theta_ms_um[1], 2)), sqrt(pow(fgaus3x->GetParameter(2), 2) - pow(theta_ms_um[2], 2))};
    TGraph *gr_emit_x = new TGraph(3, z_dist, sigmadiff_x);
    gr_emit_x->SetTitle("Sigma x as a function of z");
    gr_emit_x->GetXaxis()->SetTitle("Distance (#mum)");
    ...
    gr_emit_x->Fit("pol2");
    // In Y
    Double_t sigmadiff_y[3] = {fgaus1y->GetParameter(2), sqrt(pow(fgaus2y->GetParameter(2), 2) - pow(theta_ms_um[1], 2)), sqrt(pow(fgaus3y->GetParameter(2), 2) - pow(theta_ms_um[2], 2))};
    TGraph *gr_emit_y = new TGraph(3, z_dist, sigmadiff_y);
    gr_emit_y->SetTitle("Sigma y as a function of z");
    gr_emit_y->GetXaxis()->SetTitle("Distance (#mum)");
