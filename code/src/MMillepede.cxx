/////////////////////////////////////////////////////////////////////////////////////////
//                                                                                     //
// Class Description of MMillepede                                                     //
//                                                                                     //
...
//=============================================================================
Int_t MMillepede::InitMille(int nGlo, int nLoc, int lNStdDev,
							double lResCut, double lResCutInit)
{
   ...
   MMillepede::SetNLocalEquations(0);     // Number of local fits (starts at 0)
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SetGlobalParameters(double *param)
{
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SetGlobalParameter(int iPar, double param)
{
...
/*
 ...
 -----------------------------------------------------------
 */ 
Int_t MMillepede::SetParSigma(int iPar, double sigma)
{
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SetNonLinear(int iPar)
{
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SetIterations(double lChi2CutFac)
{
...
/*
 -----------------------------------------------------------
 CONSTF: define a constraint equation in MMillepede
 -----------------------------------------------------------
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SetGlobalConstraint(double dercs[], double lLagMult)
{ 
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SetLocalEquation(double dergb[], double derlc[], double lMeas, double lSigma)
{	
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::LocalFit(int iFit, double localParams[], Bool_t bSingleFit)
{
   ...
   Int_t nRank = MMillepede::SpmInv(fMatCLoc, fVecBLoc, fNLocalPar);
   ...
   {
	  lChi2Cut = MMillepede::Chi2DoFLim(fNStdDev, nDoF)*fChi2CutFactor;
	  if (fDebugLevel) 
   ...
   MMillepede::SpAVAt(fMatCLoc, fMatCGloLoc, fMatCGloCorr, fNLocalPar, nGloInFit);
   MMillepede::SpAX(fMatCGloLoc, fVecBLoc, fVecBGloCorr, fNLocalPar, nGloInFit);
...
/*
 ...
 parameters (estimated by MMillepede)
 
 pull[]        = array containing the corresponding pulls 
 
 -----------------------------------------------------------
 */
Int_t MMillepede::GlobalFit(double par[], double error[], double pull[])
{
   ...
   nLocFitsTot = MMillepede::GetNLocalEquations();
   
   while (fIter < fMaxIter)  // Iteration for the final loop
   {
	  
	  nLocFits = MMillepede::GetNLocalEquations();
	  ...
	  Int_t nRank = MMillepede::SpmInv(fMatCGlo, fVecBGlo, nVar);
	  ...
	  for (i=0; i<nLocFitsTot; i++) {
		 ...
		 if (fIndexAllEqs[iEqFirst] != -999) { // Fit is still OK      
			...
			//  	Int_t sc = MMillepede::LocalFit(i,localPars,0);
			// 	(sc) ? nLocFitsGood++ : fIndexAllEqs[iEqFirst] = -999;   
			MMillepede::LocalFit(i,localPars,0);
			nLocFitsGood++;
		 }
	  } // End of loop on fits
	  
	  MMillepede::SetNLocalEquations(nLocFitsGood);
	  
   } // End of iteration loop
   
   MMillepede::PrintGlobalParameters(); // Print the final results
...
/*
 ...
 -----------------------------------------------------------
 */ 
Double_t MMillepede::GetParError(int iPar) const
{
...
/*
 ...
 -----------------------------------------------------------
 */
int MMillepede::SpmInv(double matV[][fgkMaxGloPC], double vecB[], int nGlo)
{
...
//
int MMillepede::SpmInv(double matV[][fgkMaxLocalPar], double vecB[], int nLoc)
{
   ///  Obtain solution of a system of linear equations with symmetric matrix 
   ///  and the inverse (using 'singular-value friendly' GAUSS pivot)
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SpAVAt(double matV[][fgkMaxLocalPar], double matA[][fgkMaxLocalPar], double matW[][fgkMaxGlobalPar], int nLoc, int nGlo)
{
   ///  multiply symmetric N-by-N matrix from the left with general M-by-N
   ///  matrix and from the right with the transposed of the same general
   ///  matrix to form symmetric M-by-M matrix.
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::SpAX(double matA[][fgkMaxLocalPar], double vecX[], double vecY[], int nCol, int nRow)
{
   ///   multiply general M-by-N matrix A and N-vector X
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t MMillepede::PrintGlobalParameters() const
{
   ///  Print the final results into the logfile
   double lError = 0.;
   double lGlobalCor =0.;
...
/*
 ...
 ----------------------------------------------------------------
 */
double MMillepede::Chi2DoFLim(int nSig, int nDoF)
{
