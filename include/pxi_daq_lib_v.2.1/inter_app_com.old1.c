...
SInt32 OLD___IAC__PRIV_FSockSendShortMsg ( SInt32 ContextId ) {
  ...
  VTotMsgSz = VPtSock->ParMsgSz + IAC__SOCK_HEADER_TOT_SZ;
  
  memcpy ( VPtSock->PtBuffSendShortMsg, IAC__SOCK_HEADER_STR , IAC__SOCK_HEADER_STR_SZ - 1 );    // Copy string
  VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_STR_SZ] = 0;                                      // Set 0 at end of string
  memcpy ( &(VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_STR_SZ+1]), &VTotMsgSz, 4        );    // Copy TOTAL message size (W32)
  memcpy ( &(VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_TOT_SZ])  , VPtSock->ParPtMsg, VPtSock->ParMsgSz    );    // Copy user message
  
  memcpy ( &VTmpTotSz, &(VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_STR_SZ+1]), 4 );

  err_trace (( ERR_OUT, "IAC__SOCK_HEADER_STR_SZ+1 = %d", IAC__SOCK_HEADER_STR_SZ+1 ));
  err_trace (( ERR_OUT, "IAC__SOCK_HEADER_TOT_SZ   = %d", IAC__SOCK_HEADER_TOT_SZ ));
  err_trace (( ERR_OUT, "VTotMsgSz      = %d", VTotMsgSz ));
  err_trace (( ERR_OUT, "Message header = %s", VPtSock->PtBuffSendShortMsg ));
  err_trace (( ERR_OUT, "- Message size   = %d", VTmpTotSz ));
  err_trace (( ERR_OUT, "Message        = %s", &(VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_TOT_SZ]) ));
...
SInt32 OLD___IAC__PRIV_FSockSendLongMsg ( SInt32 ContextId ) {
  ...
  if ( VPtSock->ParMsgSz <= (IAC__SO_MAX_SHORT_MSG_SIZE - IAC__SOCK_HEADER_TOT_SZ) )   {
    VHeaderBlockDataSz   = VPtSock->ParMsgSz;
    VHeaderBlockTotSz    = VHeaderBlockDataSz + IAC__SOCK_HEADER_TOT_SZ;
    VDataBlocksTotSz     = 0;
    VDataBlockSz         = 0;
    VBlockNb             = 0;
    ...
    VHeaderBlockDataSz   = IAC__SO_MAX_SHORT_MSG_SIZE - IAC__SOCK_HEADER_TOT_SZ;
  ...
  memcpy ( VPtSock->PtBuffSendShortMsg, IAC__SOCK_HEADER_STR , IAC__SOCK_HEADER_STR_SZ - 1 );      // Copy header string
  VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_STR_SZ] = 0;                                        // Set 0 at end of string
  memcpy ( &(VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_STR_SZ+1]), &(VPtSock->ParMsgSz), 4      );            // Copy TOTAL message size (W32)
  memcpy ( &(VPtSock->PtBuffSendShortMsg[IAC__SOCK_HEADER_TOT_SZ])  , VPtSock->ParPtMsg, VHeaderBlockDataSz ); // Copy message
  
  VPosNextBlockInSrcBuff = VHeaderBlockDataSz;
  
  err_trace (( ERR_OUT, "Message size written in header=%d", VPtSock->ParMsgSz ));
  ...
  if ( VSzSent == SOCKET_ERROR ) {
    err_retfail ( -1, (ERR_OUT,"send () error %d - Header => %s", IAC__SOCKET_ERRNO, strerror (IAC__SOCKET_ERRNO) ));
  }
  ...
  err_trace (( ERR_OUT, "Before wainting for host ACK" ));
...
SInt32 OLD1___IAC__PRIV_FSockLookForShortRecMsgOnPortAndProcessIt ( UInt32 RecPortContextId ) {
  ...
  VMsgSz = recv ( VPtSock->ASocket, &(VPtSock->PtMsg[0]), VPtSock->MaxMsgSz, 0 );
  
  err_trace (( ERR_OUT, "MsgSz=%d", VMsgSz ));
  
  if( VMsgSz <= 0 ) {
    err_retfail ( -1, (ERR_OUT,"recv () error %d => %s", IAC__SOCKET_ERRNO, strerror (IAC__SOCKET_ERRNO) ));
  }
  
  // Set status      
        
  VPtSock->MsgSz    = VMsgSz;
  VPtSock->MsgReady = 1;
  
  // Call user function
  
  if ( VPtSock->ParUsrFunc != NULL ) {
    VPtSock->ParUsrFunc ( VPtSock->PtMsg, VPtSock->MsgSz, &(VPtSock->PtMsg[IAC__SOCK_HEADER_TOT_SZ]), VPtSock->MsgSz -  IAC__SOCK_HEADER_TOT_SZ );
  }
  
  err_trace (( ERR_OUT, "Message received => %d bytes", VMsgSz ));
  
  // Send back size of message
  
  VAckSentSz = send ( VPtSock->ASocket, (SInt8*) &VMsgSz, 4, 0 );
...
SInt32 OLD2___IAC__PRIV_FSockLookForShortRecMsgOnPortAndProcessIt ( UInt32 RecPortContextId ) {
...
SInt32 OLD1___IAC__PRIV_FSockLookForLongRecMsgOnPortAndProcessIt ( UInt32 RecPortContextId ) {
  ...
  while (1) {
    ...
    if ( strncmp ( &(VPtSock->PtMsg[VTotRecSz]), IAC__SOCK_HEADER_STR, IAC__SOCK_HEADER_STR_SZ-1 ) == 0 ) {
      
      memcpy ( &VMsgSz, &(VPtSock->PtMsg[IAC__SOCK_HEADER_STR_SZ+1]), 4 );
      ...
      err_trace (( ERR_OUT, "Message header = %s - MsgSz=%d", VPtSock->PtMsg, VMsgSz ));
    ...
    if ( VTotRecSz >= VMsgSz ) {
      ...
      if ( VPtSock->ParUsrFunc != NULL ) {
        VPtSock->ParUsrFunc ( VPtSock->PtMsg, VTotRecSz, &(VPtSock->PtMsg[IAC__SOCK_HEADER_TOT_SZ]), VTotRecSz -  IAC__SOCK_HEADER_TOT_SZ );
      }
