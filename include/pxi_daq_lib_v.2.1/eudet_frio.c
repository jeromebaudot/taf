...
Rev       : 11/05/2011
          ...
          :
          : 26/03/2013
          : - Add EFRIO__USR_VGContext.ParCrIndexFile handling
          :
          : 29/10/2013 - Lib V3.0
          : - Add COM SPI interface to clk dirstib board SP
          : - ONLY on version V3.0 defined by CC => EFRIO__V30 in the DLL cpp file
          :
Doc date  : 07/11/2010
Author    : Gilles CLAUS
E-mail    : gilles.claus@ires.in2p3.fr
...
SInt32 EFRIO__FBeginExt ( SInt8 MapsName, SInt8 ErrorLogLvl, char* ErrorLogFile, SInt8 MsgLogLvl, char* MsgLogFile ) {
  ...
  switch ( MapsName ) {
    ...
    case ASIC__MI26 : {
      EFRIO__VGJtagChip = EFRIO__JTAG_MI26; // 0 => Mi26, 1 => Ultimate
      break; }

    case ASIC__ULT1 : {
      EFRIO__VGJtagChip = EFRIO__JTAG_ULT1; // 0 => Mi26, 1 => Ultimate
      break; }
	  
	case ASIC__FSBB0:{
	  EFRIO__VGJtagChip = EFRIO__JTAG_FSBB0;
	  break; }
  ...
  // Set version information ( only for version >= 2.0 )
    ...
    #ifdef EFRIO__V30
      VPtCont->VersionMajor = 3;
      VPtCont->VersionMinor = 0;
    
      sprintf ( VPtCont->VersionCmt, "Extension of V2.0 to SALAT (COM interface / SPI clk distri)" );
    #endif
  ...
  switch ( MapsName ) {
    ...
    case ASIC__ULT1 : {
      ...
      break; }
    case ASIC__FSBB0 : {
      VPtBoard->DmaHostSz = 0;//((FSBB0__ZS_FFRAME_MODE_1X160MHZ_W8_SZ * 2 * (EFRIO__MAX_ASIC_NB_FOR_DMA + 1) * VPtBoard->FrameNbPerAcq) / (1024 * 1024)) + 5;
      break; }    
    default : {
      err_retfail ( -1, (ERR_OUT,"Unknown MAPS name = %d => Abort", MapsName) );
      break; }
  ...
  switch (EFRIO__VGJtagChip){
	case EFRIO__JTAG_MI26:{
	...
	case EFRIO__JTAG_FSBB0:{
		VHrComErr =  CoFSBBM0COM::Create( EFRIO_VGJtagFsbb0 );
...
#ifdef EFRIO__V30
  SPICD_FBegin ( (SInt8) ErrorLogLvl, ErrorLogFile );
  SPICD_FStartCom ();
  Sleep ( 1000 );
  SPICD_FSendConf ();
  Sleep ( 1000 );
  ...
  err_error (( ERR_OUT, "------------------------------------------------------------------" ));
  err_error (( ERR_OUT, "Max Acq Nb = %d", FIL__TCStreamFile_MAX_BLOC_NB_IN_VAR_BLOC_SZ_MODE ));
  err_error (( ERR_OUT, "------------------------------------------------------------------" ));
  

  #ifdef EFRIO__FREE_MI26_NB
    err_error (( ERR_OUT, "----------------------------------------------------------------------------" ));
    err_error (( ERR_OUT, "! Mi26 number is free ( mode EUDET 2 & EUDET 3 ) = Not limited to 1, 5, 6, 8" ));
    err_error (( ERR_OUT, "! BUT current implementation will slow down acquisition !" ));
  #else
    err_error (( ERR_OUT, "----------------------------------------------------------------------------" ));
    err_error (( ERR_OUT, "! Mi26 number can be ONLY 1, 5, 6, 8" ));
    err_error (( ERR_OUT, "! Because data demultiplexing is hard coded, in order to achieve the shortest execution time" ));  
  #endif


  #ifdef EFRIO__CREATE_INDEX_FILE
  
    if ( VPtUsrCont->ParCrIndexFile ) {
      err_error (( ERR_OUT, "" ));
      err_error (( ERR_OUT, "-------------------------------------------------------------------------------" ));
      err_error (( ERR_OUT, "! Index file (needed for DUT DAQ interface) is ENABLED by line cmd param      !" ));
      err_error (( ERR_OUT, "! It will slow down the acquisition                                           !" ));
      err_error (( ERR_OUT, "! Maximum trigger rate (6 x Mimosa 26 saturated) without loosing frames :     !" ));
      err_error (( ERR_OUT, "! - Saving data on RAID     => FTrig max = 1 KHz                              !" ));
      err_error (( ERR_OUT, "! - Saving data on CPU disk => FTrig max ~ 500 Hz                             !" ));
    }
  
    else {
      err_error (( ERR_OUT, "-------------------------------------------------------------------------------" ));
      err_error (( ERR_OUT, "! Index file (needed for DUT DAQ interface) is DISABLED by line cmd param     !" ));
      err_error (( ERR_OUT, "! Maximum trigger rate (6 x Mimosa 26 saturated) without loosing frames :     !" ));
      err_error (( ERR_OUT, "! - Saving data on RAID     => FTrig max > 8 KHz (100 KHz & 1 MHz tested)     !" ));
      err_error (( ERR_OUT, "! - Saving data on CPU disk => FTrig max ~ 500 Hz                             !" ));
      err_error (( ERR_OUT, "-------------------------------------------------------------------------------" ));   
    }
  ...
  #else
    err_error (( ERR_OUT, "-------------------------------------------------------------------------------" ));
    err_error (( ERR_OUT, "! This DAQ version DOESN'T create the index file needed for DUT DAQ interface !" ));
    err_error (( ERR_OUT, "! Maximum trigger rate (6 x Mimosa 26 saturated) without loosing frames :     !" ));
    err_error (( ERR_OUT, "! - Saving data on RAID     => FTrig max > 8 KHz (100 KHz & 1 MHz tested)     !" ));
    err_error (( ERR_OUT, "! - Saving data on CPU disk => FTrig max ~ 500 Hz                             !" ));
    err_error (( ERR_OUT, "-------------------------------------------------------------------------------" ));
  ...
  msg (( MSG_OUT, "EFRIO__FBeginExt end" ));
...
SInt32 EFRIO__FBegin ( SInt8 ErrorLogLvl, char* ErrorLogFile, SInt8 MsgLogLvl, char* MsgLogFile ) {
  ...
  return ( EFRIO__FBeginExt ( ASIC__FSBB0, ErrorLogLvl, ErrorLogFile, MsgLogLvl, MsgLogFile ) );
}
...
SInt32 EFRIO__FConfRun ( SInt8 Mi26Nb, SInt32 RunNo, SInt32 TotEvNb, SInt32 EvNbPerFile, SInt32 FrameNbPerAcq, SInt8 DataTransferMode, SInt8 TrigMode, char* DestDir, char* FileNamePrefix, SInt8 SaveToDisk, SInt8 SendOnEth, SInt8 SendOnEthPCent, char* JtagFileName ) {

  SInt32 VRet;

  VRet = EFRIO__FConfRunExt ( ASIC__FSBB0, Mi26Nb, RunNo, TotEvNb, EvNbPerFile, FrameNbPerAcq, DataTransferMode, TrigMode, DestDir, FileNamePrefix, SaveToDisk, SendOnEth, SendOnEthPCent, JtagFileName );
...
SInt32 EFRIO__FConfRunExt ( SInt8 MapsName, SInt8 MapsNb, SInt32 RunNo, SInt32 TotEvNb, SInt32 EvNbPerFile, SInt32 FrameNbPerAcq, SInt8 DataTransferMode, SInt8 TrigMode, char* DestDir, char* FileNamePrefix, SInt8 SaveToDisk, SInt8 SendOnEth, SInt8 SendOnEthPCent, char* JtagFileName ) {
  ...
  sprintf ( VPtCont->RunCont.ParJtagFileName  , "%s", JtagFileName );
  ...
  #ifdef EFRIO__CREATE_INDEX_FILE
    if ( VPtUsrCont->ParCrIndexFile ) {
      sprintf ( VPtCont->RunCont.InfIndexFileName, "%s\\%s%d_index.bin", VPtCont->RunCont.ParDestDir, VPtCont->RunCont.ParFileNamePrefix, VPtCont->RunCont.ParRunNo);
    }
  #endif
  ...
  VPtBoard->BoardId = 0;
  // 30/09/2014 - MS : changed the setting of the AsicName
  switch ( MapsName ) {
    ...
    case ASIC__ULT1 : {
		sprintf ( VPtBoard->AsicName, "%s", "Ultimate 1" );
		VPtBoard->DataClkFrequency         = 80; // 80 MHz
      break; }
	  
	case ASIC__FSBB0:{
		sprintf ( VPtBoard->AsicName, "%s", "FSBB 0" );
		VPtBoard->DataClkFrequency         = 160; // 160 MHz
	  break; }
...
Rev       : 15/02/2011
          ...
          :
          : 10/07/2012
          : - Create run index file
          :
          : 09/11/2012
          : - Automatic creation of run directory
          :
          : 26/03/2013
          : - Add EFRIO__USR_VGContext.ParCrIndexFile handling
          :          
...
SInt32 EFRIO__FStartSavingOnFile () {
  ...
  if ( VPtRunCont->ParSaveOnDisk > 0 ) {
    ...
    msg (( MSG_OUT, "EFRIO__FStartSavingOnFile () => Dest dir = %s", VPtRunCont->ParDestDir ));
    
    // 09/11/12
    
    if ( DirectoryExists ( VPtRunCont->ParDestDir ) == True ) {
      msg (( MSG_OUT, "Dest dir = %s already exists", VPtRunCont->ParDestDir ));
    }
    
    else {
      msg (( MSG_OUT, "Dest dir = %s doesn't exist => Try to create it", VPtRunCont->ParDestDir ));
        
      err_retfail ( FIL_FMkDir ( VPtRunCont->ParDestDir ), (ERR_OUT,"Abort : Directory %s creation failed !", VPtRunCont->ParDestDir ) );     
    }
    ...
    #ifdef EFRIO__CREATE_INDEX_FILE
      if ( VPtUsrCont->ParCrIndexFile ) {         
        VRet = FIL_FIndexFileTelCreate ( VPtRunCont->InfIndexFileName );
        err_retfail ( VRet, (ERR_OUT,"Index file %s creation failed => %s", VPtRunCont->InfIndexFileName, _strerror ( "System says :" ) ));
      }
    #endif      
...
//    err_error (( ERR_OUT, "***********************************************" ));
//    err_error (( ERR_OUT, "TRACE => Saving AcqId=%d - %d Frames", VPtRunCont->ResAcqCnt, VPtFrList->TotFrameNb ));
//    err_error (( ERR_OUT, "***********************************************" ));

    if ( VPtRunCont->ResAcqCnt == 1 ) {
      ViEv = 0;
    }
    
    for ( ViFr=0; ViFr < VPtFrList->TotFrameNb; ViFr++ ) {

//      err_error (( ERR_OUT, "EvId=%d - AcqId=%d - Fr=%d : Header=%x [H] - FrCnt=%d", ViEv, VPtRunCont->ResAcqCnt - 1, ViFr, VPtFrList->AFramePtr[ViFr]->Header.AMapsHeader[0], VPtFrList->AFramePtr[ViFr]->Header.AMapsFrameCnt[0] ));
  
      #ifdef EFRIO__CREATE_INDEX_FILE
        if ( VPtUsrCont->ParCrIndexFile ) {              
          FIL_FIndexFileTelAddEv ( ViEv /* EvId */, VPtRunCont->ResAcqCnt - 1 /* AcqId */, ViFr /* FrId */, VPtFrList->AFramePtr[ViFr]->Header.AMapsFrameCnt[0] /* EvTag */, VPtFrList->AFramePtr[ViFr]->Header.AMapsTrigInfo[0], VPtFrList->AFramePtr[ViFr]->Header.AMapsTrigInfo[1], VPtFrList->AFramePtr[ViFr]->Header.AMapsTrigInfo[2], VPtFrList->AFramePtr[ViFr]->Header.TriggerNb, 0 /* SpareU32 */ );
        }
      #endif
...
Rev       : 15/05/2013
          ...
          :
          : 08/11/2013
          : - Update test context field ResErrOnCurrentAcq with errors count
          :
Doc date  : 20/08/2009
Author    : Gilles CLAUS
E-mail    : gilles.claus@ires.in2p3.fr
...
SInt32 EFRIO__MI26_FChkFrameLight ( SInt16 FuncId, SInt32 CurFrame, EFRIO__TFrame* PtFrame, SInt8 Mi26Nb ) {
  ...
  for ( ViMi26=0; ViMi26 < VMi26Nb; ViMi26++ ) {
    ...
    // Check frame counter => Compare all Mimosa 26 frame counters to the one of Mimosa26 [0]
...
SInt32 EFRIO__MI26_FPrintFrameExtraTrig ( SInt32 AcqId, SInt32 CurFrame, EFRIO__TFrame* PtFrame, SInt8 Mi26Nb ) {
  ...
  UInt32*              VPtAW32;
  SInt8                ViMi26ExtraTrig;
  SInt32               VOneMapsW32Sz;
  SInt32               VExtraTrigW32DataOffset;
  SInt32               ViDataW32;
  SInt32               VTrig1Cnt;
  SInt32               VTrig2Cnt;
  
  
  // msg (( MSG_OUT, "------------------- EFRIO__MI26_FPrintFrameExtraTrig -------------------" ));
  
  // Check MAPS nb 
    
  if ( Mi26Nb < 2 ) {
    err_retfail ( -1, (ERR_OUT,"Mi26Nb=%d => Abort, no extra trig channel", Mi26Nb )); // Check MAPS nb to test
  }
  ...
  // Get index of Mi26 used as extra trigger channel
  
  ViMi26ExtraTrig = Mi26Nb - 1;
  
  // Init ptr
  
  VPtFrHd = &PtFrame->Header;
  VPtData = &PtFrame->Data;
  
  VOneMapsW32Sz           = VPtData->OneMapsSz / 4;
  VExtraTrigW32DataOffset = ViMi26ExtraTrig * VOneMapsW32Sz;
  
  VPtAW32 = &PtFrame->Data.ADataW32[VExtraTrigW32DataOffset];
      
  // Debug print

  // msg (( MSG_OUT, "Frame [%d] : TotSz=%d W8 - OneMapsSz=%d W8 - VExtraTrigW32DataOffset=%d", CurFrame, VPtData->TotSz, VPtData->OneMapsSz, VExtraTrigW32DataOffset ));
  
  
  // List trigger accepted by Mi32 received on D00
    
  if ( (VPtFrHd->AMapsHeader[ViMi26ExtraTrig] & 0x0000FFFF) != 0 ) {
    ++VTrig1Cnt;
    msg (( MSG_OUT, "##Mi32 Trig Line 0 Header=%X", VPtFrHd->AMapsHeader[ViMi26ExtraTrig] ));
  }
  
  if ( (VPtFrHd->AMapsFrameCnt[ViMi26ExtraTrig] & 0x0000FFFF) != 0 ) {
    ++VTrig1Cnt;
    msg (( MSG_OUT, "##Mi32 Trig Line 1 FrCnt=%X", VPtFrHd->AMapsFrameCnt[ViMi26ExtraTrig] ));
  }
  
  if ( (VPtFrHd->AMapsDataLength[ViMi26ExtraTrig] & 0x0000FFFF) != 0 ) {
    ++VTrig1Cnt;
    msg (( MSG_OUT, "##Mi32 Trig Line 2 DataLength=%X", VPtFrHd->AMapsDataLength[ViMi26ExtraTrig] ));
  }
   
  if ( (VPtFrHd->AMapsTrailer[ViMi26ExtraTrig] & 0x0000FFFF) != 0 ) {
    ++VTrig1Cnt;
    msg (( MSG_OUT, "##Mi32 Trig Line 573 Trailer=%X", VPtFrHd->AMapsTrailer[ViMi26ExtraTrig] ));
  }
    
  // Search in data

  for ( ViDataW32=0; ViDataW32 < VOneMapsW32Sz; ViDataW32++ ) {
    
    if ( (VPtAW32[ViDataW32] & 0x0000FFFF) != 0 ) {
      ++VTrig1Cnt;
      msg (( MSG_OUT, "##Mi32 Trig Line %d - Data[%.4d]=%X", 3 + ViDataW32, ViDataW32, VPtAW32[ViDataW32] ));
    }
    
  }

  // List trigger from scintillator received on D01
  
  if ( (VPtFrHd->AMapsHeader[ViMi26ExtraTrig] & 0xFFFF0000) != 0 ) {
    ++VTrig2Cnt;
    msg (( MSG_OUT, "##Tot Trig Line 0 Header=%X", VPtFrHd->AMapsHeader[ViMi26ExtraTrig] ));
  }
  
  if ( (VPtFrHd->AMapsFrameCnt[ViMi26ExtraTrig] & 0xFFFF0000) != 0 ) {
    ++VTrig2Cnt;
    msg (( MSG_OUT, "##Tot Trig Line 1 FrCnt=%X", VPtFrHd->AMapsFrameCnt[ViMi26ExtraTrig] ));
  }
  
  if ( (VPtFrHd->AMapsDataLength[ViMi26ExtraTrig] & 0xFFFF0000) != 0 ) {
    ++VTrig2Cnt;
    msg (( MSG_OUT, "##Tot Trig Line 2 DataLength=%X", VPtFrHd->AMapsDataLength[ViMi26ExtraTrig] ));
  }
  
  if ( (VPtFrHd->AMapsTrailer[ViMi26ExtraTrig] & 0xFFFF0000) != 0 ) {
    ++VTrig2Cnt;
    msg (( MSG_OUT, "##Tot Trig Line 573 Trailer=%X", VPtFrHd->AMapsTrailer[ViMi26ExtraTrig] ));
  }
  ...
  for ( ViDataW32=0; ViDataW32 < VOneMapsW32Sz; ViDataW32++ ) {
    ...
    if ( (VPtAW32[ViDataW32] & 0xFFFF0000) != 0 ) {
      ++VTrig2Cnt;
      msg (( MSG_OUT, "##Tot Trig Line %d - Data[%.4d]=%X", 3 + ViDataW32, ViDataW32, VPtAW32[ViDataW32] ));
    }
    
  }
  

  if ( (VTrig1Cnt != 0) || (VTrig2Cnt != 0) ) {
    msg (( MSG_OUT, "AcqId=%d Frame [%d] : %d Mi32 Accepted triggers - %d Total triggers ", AcqId, CurFrame, VTrig1Cnt, VTrig2Cnt ));
    msg (( MSG_OUT, "------------------------------------------------------------" ));
  }
...
/* DOC_FUNC_BEGIN */
/* ===================================================================================
Prototype : SInt32 EFRIO__MI26_FFRioAcqDeserDataEudet2Mode5Mi26 (
          :   SInt32 BoardId , UInt32*  PtSrcW32, SInt32 EltNb,
          :   SInt32 AcqStatus, SInt32 TrigStatus,  SInt8 DataConvertMode )
          :
...
Level     : This is a user level function.
Date      : 28/08/2012
          : Copy EFRIO__MI26_FFRioAcqDeserDataEudet2Mode6Mi26 to create EFRIO__MI26_FFRioAcqDeserDataEudet2Mode5Mi26.
Rev       :
          : 16/02/2011
          : - Update ResEventCnt = ResFrameCnt, set to 0 before
          :
...
SInt32 EFRIO__MI26_FFRioAcqDeserDataEudet2Mode5Mi26 ( SInt32 BoardId, UInt32* PtSrcW32, SInt32 EltNb, SInt32 AcqStatus, SInt32 TrigStatus, SInt8 DataConvertMode, SInt8 TriggerHandlingMode ) {
  ...
  SInt32               ViFrame;
  SInt32               V6iFrame;
  UInt32               VADataLengthField[5];
  UInt32               VADataLengthW8[5];
  UInt16               VADataLengthW16[5];
  UInt32               VADataLengthW32[5];
  UInt32               VDataLengthW8Max;
  UInt32               VDataLengthW16Max;
  UInt32               VDataLengthW32Max;
  UInt32               VDataLengthW32ToCpy;
  ...
  VFrameNbFromBoardDrv = ( EltNb / MI26__ZS_FFRAME_MODE_2X80MHZ_W16_SZ ) / 6; // Divide by 6 because of extral channel
  ...
  for ( ViFrame=0; ViFrame < VPtBoard->FrameNbPerAcq; ViFrame++ ) {
    ...
    // err_error (( ERR_OUT, "$$$ Header [0]=%x - [1]=%x - [2]=%x - [3]=%x - [4]=%x", VPtFrame->Header.AMapsHeader[0], VPtFrame->Header.AMapsHeader[1], VPtFrame->Header.AMapsHeader[2], VPtFrame->Header.AMapsHeader[3], VPtFrame->Header.AMapsHeader[4] ));
    ...
    // err_error (( ERR_OUT, "### Data length [0]=%d - [1]=%d - [2]=%d - [3]=%d - [4]=%d", VADataLengthW16[0], VADataLengthW16[1], VADataLengthW16[2], VADataLengthW16[3], VADataLengthW16[4] ));
    
    VDataLengthW16Max = MATH_FUInt16Max ( VADataLengthW16, 5 );
    ...
    if ( VDataLengthW16Max > 2304 ) {
      ...
      for ( ViMi26ChkDataLength = 0; ViMi26ChkDataLength < 5; ViMi26ChkDataLength++ ) {
      ...
      memset ( VADataLengthW8 , 0, 5 * sizeof (VADataLengthW8[0] ) );
      memset ( VADataLengthW16, 0, 5 * sizeof (VADataLengthW16[0]) );
      memset ( VADataLengthW32, 0, 5 * sizeof (VADataLengthW32[0]) );
    ...
    VDataLengthW32ToCpy = VDataLengthW32Max * 5;
    VDataLengthW8ToCpy  = VDataLengthW32ToCpy * 4;
    ...
    VPtEChanSrcW32 += 5; // Bypass Mi26 x 5 data
    ...
    VPtFrame->Header.AMapsTrailer[0] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6iFrame) + 18 + (6 * VADataLengthW32[0])];    // 21 = 7 x 3 Fields nb before first data = Header, Frame cnt, Data length
    ++ViSrcW32;
    ...
    // err_error (( ERR_OUT, "$$$ Trailer [0]=%x - [1]=%x - [2]=%x - [3]=%x - [4]=%x", VPtFrame->Header.AMapsTrailer[0], VPtFrame->Header.AMapsTrailer[1], VPtFrame->Header.AMapsTrailer[2], VPtFrame->Header.AMapsTrailer[3], VPtFrame->Header.AMapsTrailer[4] ));
    ...
    EFRIO__MI26_FChkFrameLight ( 0 /* FuncId */ , ViFrame, VPtFrame, 5 /* Mi26Nb */ );
...
/* DOC_FUNC_BEGIN */
/* ===================================================================================
Prototype : SInt32 EFRIO__MI26_FFRioAcqDeserDataEudet3Mode5Mi26 (
          :   SInt32 BoardId , UInt32*  PtSrcW32, SInt32 EltNb,
          :   SInt32 AcqStatus, SInt32 TrigStatus,  SInt8 DataConvertMode )
          :
...
Level     : This is a user level function.
Date      : 28/08/2012
          : => Cpy of EFRIO__MI26_FFRioAcqDeserDataEudet3Mode6Mi26 to make EFRIO__MI26_FFRioAcqDeserDataEudet3Mode5Mi26
          :
Rev       : 30/12/2010
          : - Add demux of data part if EFRIO__DEMUX_MI26_DATA_PART defined in eudet_frio.def
          :
...
SInt32 EFRIO__MI26_FFRioAcqDeserDataEudet3Mode5Mi26 ( SInt32 BoardId, UInt32* PtSrcW32, SInt32 EltNb, SInt32 AcqStatus, SInt32 TrigStatus, SInt8 DataConvertMode, SInt8 TriggerHandlingMode ) {
  ...
  for ( VFrameId=0; VFrameId < VPtBoard->FrameNbPerAcq; VFrameId++ ) {
    ...
    V6FrameId = 6 * VFrameId;
    
    if ( TrigStatus == 0 ) {
      VZero  = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6FrameId) + 18 + (6 * MI26__ZS_FFRAME_RAW_MAX_W32) +  6];
    }
  ...
  */
  ...
  for ( ViFrameWithTrig=0; ViFrameWithTrig < VFrameWithTrigCnt; ViFrameWithTrig++ ) {
    ...
    VEChanTrigFieldCnt = 0;
    ViSrcW32           = VFrameId * 6 * MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32;
    ...
    VPtFrame->Header.AMapsTrailer[0] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6FrameId) + 18 + (6 * VADataLengthW32[0])];    // 18 = 6 x 3 Fields nb before first data = Header, Frame cnt, Data length
    ++ViSrcW32;
    
    VPtFrame->Header.AMapsTrailer[1] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6FrameId) + 18 + 1 + (6 * VADataLengthW32[1])];
    ++ViSrcW32;
    
    VPtFrame->Header.AMapsTrailer[2] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6FrameId) + 18 + 2 + (6 * VADataLengthW32[2])];
    ++ViSrcW32;
    
    VPtFrame->Header.AMapsTrailer[3] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6FrameId) + 18 + 3 + (6 * VADataLengthW32[3])];
    ++ViSrcW32;
    
    VPtFrame->Header.AMapsTrailer[4] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6FrameId) + 18 + 4 + (6 * VADataLengthW32[4])];
    ++ViSrcW32;
    ...
    VZero2 = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V6FrameId) + 18 + (6 * MI26__ZS_FFRAME_RAW_MAX_W32) + 12];
    ...
    EFRIO__MI26_FChkFrameLight ( 0 /* FuncId */ , VFrameId, VPtFrame, 5 /* Mi26Nb */ );
...
/* DOC_FUNC_BEGIN */
/* ===================================================================================
Prototype : SInt32 EFRIO__MI26_FFRioAcqDeserDataEudet3ModeNMi26 (
          :   SInt8 Mi26Nb, SInt32 BoardId , UInt32*  PtSrcW32, SInt32 EltNb,
          :   SInt32 AcqStatus, SInt32 TrigStatus,  SInt8 DataConvertMode )
          :
Goal      : Flex RIO readout function for N Mi26 in EUDET mode 3
          :
          : Read data of one acquisition from Flex RIO, format them in EUDET mode 3
          : by adding extra information and fill PC RAM buffer.
          :
...
SInt32 EFRIO__MI26_FFRioAcqDeserDataEudet3ModeNMi26 ( SInt8 Mi26Nb, SInt32 BoardId, UInt32* PtSrcW32, SInt32 EltNb, SInt32 AcqStatus, SInt32 TrigStatus, SInt8 DataConvertMode, SInt8 TriggerHandlingMode ) {
  ...
  err_trace (( ERR_OUT, "EFRIO__MI26_FFRioAcqDeserDataEudet3ModeNMi26 (Mi26Nb=%d, P=%x, EltNb=%d)", Mi26Nb, PtSrcW32, EltNb ));
  ...
  VFrameNbFromBoardDrv = ( EltNb / MI26__ZS_FFRAME_MODE_2X80MHZ_W16_SZ ) / VMi26NbP1; // Divide by 9 because of extral channel
  ...
  for ( VFrameId=0; VFrameId < VPtBoard->FrameNbPerAcq; VFrameId++ ) {
    ...
    V9FrameId = VMi26NbP1 * VFrameId;
    
    // V9FrameId = 9 * VFrameId;

    if ( TrigStatus == 0 ) {
      VZero  = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V9FrameId) + (VMi26NbP1 * 3) + (VMi26NbP1 * MI26__ZS_FFRAME_RAW_MAX_W32) +  VMi26NbP1]; // 27 = 9 x 3 Fields nb before first data = Header, Frame cnt, Data length
    }                                                                                                                                               //  9 = 9 x 1 Trailer
    ...
    // BUT first read it from board RAM in order to get real execution time
      ...
      VZero  = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V9FrameId) + (VMi26NbP1 * 3) + (VMi26NbP1 * MI26__ZS_FFRAME_RAW_MAX_W32) +  VMi26NbP1];
      VZero = (TrigStatus << 16);
    }
  ...
  */
  ...
  for ( ViFrameWithTrig=0; ViFrameWithTrig < VFrameWithTrigCnt; ViFrameWithTrig++ ) {
    ...
    VFrameId           = VPtFrameWithTrigList[ViFrameWithTrig];
    VEChanTrigFieldCnt = 0;
    ViSrcW32           = VFrameId * VMi26NbP1 * MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32;
    ...
    */
    ...
    if ( VDataLengthW16Max > 2304 ) {
      ...
      memset ( VADataLengthW8 , 0, Mi26Nb /* 8 */ * sizeof (VADataLengthW8[0] ) );
      memset ( VADataLengthW16, 0, Mi26Nb /* 8 */ * sizeof (VADataLengthW16[0]) );
      memset ( VADataLengthW32, 0, Mi26Nb /* 8 */ * sizeof (VADataLengthW32[0]) );
      ...
      for ( ViMi26=0; ViMi26 < Mi26Nb /* 8 */; ViMi26++ ) {
        VADataLengthW8[ViMi26]  = VADataLengthW16[ViMi26] * 2;
        VADataLengthW32[ViMi26] = VADataLengthW16[ViMi26] / 2;
      }
    ...
    */
    ...
    VDataLengthW32ToCpy = VDataLengthW32Max * Mi26Nb /* 8 */;
    VDataLengthW8ToCpy  = VDataLengthW32ToCpy * 4;
    ...
    */
    ...
    VPtEChanSrcW32 += Mi26Nb; // Bypass Mi26 x 8 data
    ...
    for ( ViMi26=0; ViMi26 < Mi26Nb; ViMi26++ ) {
      VPtFrame->Header.AMapsTrailer[ViMi26] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V9FrameId) + (3 * VMi26NbP1) + ViMi26 + (VMi26NbP1 * VADataLengthW32[ViMi26])];
      ++ViSrcW32;
    }
    
        
    // 27 = 9 x 3 Fields nb before first data = Header, Frame cnt, Data length
    /*
    VPtFrame->Header.AMapsTrailer[0] = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V9FrameId) + 27 + (9 * VADataLengthW32[0])];    
    ++ViSrcW32;
    ...
    */
    ...
    if ( TrigStatus == 0 ) {
      ...
      VZero  = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V9FrameId) + (3 * VMi26NbP1) + (VMi26NbP1 * MI26__ZS_FFRAME_RAW_MAX_W32) +  VMi26NbP1];
    ...
    VZero2 = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V9FrameId) + (3 * VMi26NbP1) + (VMi26NbP1 * MI26__ZS_FFRAME_RAW_MAX_W32) + (2 * VMi26NbP1)];
    
    // VZero2 = PtSrcW32[(MI26__ZS_FFRAME_RAW_TOTAL_SZ_W32 * V9FrameId) + 27 + (9 * MI26__ZS_FFRAME_RAW_MAX_W32) + 18];
    
    
    ViSrcW32 += (2 * VMi26NbP1); // 9 times 2 zero fields = 18
...
SInt32 EFRIO__MI26_FFRioAcqDeserDataEudet3ModeNMi26ExtaTrigInfo ( SInt8 Mi26Nb, SInt32 BoardId, UInt32* PtSrcW32, SInt32 EltNb, SInt32 AcqStatus, SInt32 TrigStatus, SInt8 DataConvertMode, SInt8 TriggerHandlingMode ) {
  ...
  SInt16               ViFrameWithTrig;
  SInt32               V9FrameId;
  UInt32               VADataLengthField[17];
  UInt32               VADataLengthW8[17];
  UInt16               VADataLengthW16[17];
  UInt32               VADataLengthW32[17];
  UInt32               VDataLengthW8Max;
  UInt32               VDataLengthW16Max;
  UInt32               VDataLengthW32Max;
  UInt32               VDataLengthW32ToCpy;
  ...
  */
  ...
  for ( ViFrameWithTrig=0; ViFrameWithTrig < VFrameWithTrigCnt; ViFrameWithTrig++ ) {
    ...
    */
    ...
    // 14/06/13 : Search max size on Mi26Nb - 1 because last one is the extra trigger info
    
    VDataLengthW16Max = MATH_FUInt16Max ( VADataLengthW16, VMi26NbM1 );
    
    // msg (( MSG_OUT, "VDataLengthW16Max [0..%d]=%d", VMi26NbM1 - 1, VDataLengthW16Max ));
    ...
    */
    ...
    // Test done before 14/06/13 "if ( VDataLengthW16Max > 2304 ) {" => Don't remember why 2304, should be 1140 W16 => ???
    // Rq : 2304 = 1152 W16 x 2 = Total size (including non data fields) in W8 => It may be the explanation
    // but it confirms that it's not the good value
    
    // 14/06/13 : Modify the test => use the good Max W16 value
    //
    // If one Mi26 data field is out of normal range => Force all of them to 0 in order to
    // - avoid SW crash during frame processing with corrupted data
    // - indicate that there is a problem with this event => data size set to 0 => event not processed
    
    if ( VDataLengthW16Max > 1140 ) {
    ...
    // 14/06/13
    // If data size fields of Mi26Nb connected to DAQ (Mi26Nb - 1) is OK
    // => Set maximum data size variable to MAXIMUM possible value
    // => Real data size (from Mi26 data size fields) to Mi26 0 to Mi26 nb - 1
    // => Set data size to MAXIMUM possible value for the last Mi26 which is extra trigger channel
    // because for extra trigger acquisition, we must process the full frame
    
    else {

      // Set maximum size variable to MAXIMUM possible size
      
      VDataLengthW16Max = 1140;
      VDataLengthW8Max  = VDataLengthW16Max * 2;
      VDataLengthW32Max = VDataLengthW16Max / 2;
      
      // Set size of Mi26 No 0 to NbMax  - 1

      for ( ViMi26=0; ViMi26 < VMi26NbM1 /* 8 */; ViMi26++ ) {
        VADataLengthW8[ViMi26]  = VADataLengthW16[ViMi26] * 2;
        VADataLengthW32[ViMi26] = VADataLengthW16[ViMi26] / 2;
        
        // msg (( MSG_OUT, "VADataLengthW16[%d]=%d", ViMi26, VADataLengthW16[ViMi26] ));
      }

      // Set size of last Mi26 = extra trigger channel, to MAXIMUM possible size

      VADataLengthW16[VMi26NbM1] = VDataLengthW16Max;
      VADataLengthW8[VMi26NbM1]  = VDataLengthW8Max;
      VADataLengthW32[VMi26NbM1] = VDataLengthW32Max;

      // msg (( MSG_OUT, "VADataLengthW16[%d]=%d", VMi26NbM1, VADataLengthW16[VMi26NbM1] ));
    
      
      // msg (( MSG_OUT, "VDataLengthW16Max = [6] =%d", VDataLengthW16Max ));      
    }
    
    
    for ( ViMi26=0; ViMi26 < VMi26NbM1; ViMi26++ ) {
      VPtFrame->Header.AMapsDataLength[ViMi26] = VADataLengthW8[ViMi26];
    }
    
    // Extra trigger channel => Copy only link D00
    
    VPtFrame->Header.AMapsDataLength[VMi26NbM1] = (VADataLengthField[VMi26NbM1] & 0x0000FFFF);        
    ...
    */
    ...
    // 14/06/13 : Force to test only NbMax Mi26 - 1, because the last channel is not a Mi26 but the extra trigger info
    
    EFRIO__MI26_FChkFrameLight ( 0 /* FuncId */ , VFrameId, VPtFrame, VMi26NbM1 /* Mi26Nb */ );
    
    // 17/06/13 Print extra trig
       
    EFRIO__MI26_FPrintFrameExtraTrig ( VAcqId /* AcqId */ , VFrameId, VPtFrame, Mi26Nb /* Mi26Nb */ );
  ...
  while (1) {
    ...
    if ( DataConvertMode == EFRIO__TRF_MODE_EUDET_2__TRG_CHAN__SEND_ALL_FRAMES ) {
        ...
        switch ( Mi26Nb ) {
          ...
          case 5 : {
            VRet = EFRIO__MI26_FFRioAcqDeserDataEudet2Mode5Mi26 ( BoardId, PtSrcW32AsPt, EltNb, AcqStatus, TrigStatus, DataConvertMode, TriggerHandlingMode );
            break; }
    ...
    if ( DataConvertMode == EFRIO__TRF_MODE_EUDET_3__TRG_CHAN__SEND_FRAMES_WITH_TRIG ) {
        ...
        if ( VPtUsrContext->ParReadoutMode == 0) {
          ...
          VRet = EFRIO__MI26_FFRioAcqDeserDataEudet3ModeNMi26 ( Mi26Nb, BoardId, PtSrcW32AsPt, EltNb, AcqStatus, TrigStatus, DataConvertMode, TriggerHandlingMode );
        }
        
        else {
          // msg (( MSG_OUT, "Readout WITH extra trigger info" ));
          VRet = EFRIO__MI26_FFRioAcqDeserDataEudet3ModeNMi26ExtaTrigInfo ( Mi26Nb, BoardId, PtSrcW32AsPt, EltNb, AcqStatus, TrigStatus, DataConvertMode, TriggerHandlingMode );
        }
        ...
        switch ( Mi26Nb ) {
          ...
          case 5 : {
            VRet = EFRIO__MI26_FFRioAcqDeserDataEudet3Mode5Mi26 ( BoardId, PtSrcW32AsPt, EltNb, AcqStatus, TrigStatus, DataConvertMode, TriggerHandlingMode );
            break; }
...
SInt32 EFRIO__FJtagLoadFile ( char* FileName ) {
  ...
  msg (( MSG_OUT, "### EFRIO__FJtagLoadFile (FileName=%s)", FileName ));
...
SInt32 EFRIO__MI26_FJtagLoadDefFile () {
  ...
  sprintf ( VFileName, "%s", "c:\\ccmos_sctrl\\Mimosa26_jtag\\config_files\\Default.mcf" );
  
  msg (( MSG_OUT, "### EFRIO__MI26_FJtagLoadDefFile (FileName=%s)", VFileName ));
  
  VRet = EFRIO__MI26_FJtagLoadFile (VFileName);
...
Rev       : 04/02/2011
          ...
          :
          : 08/11/2013
          : - Add dev 0 selection to fix "JTAG GUI panel bug" when using only one MAPS
          :   following an advise from KKJ, but it solves nothing ... let in place, in case of ...
          :          
Doc date  : 09/11/2010
Author    : Gilles CLAUS
E-mail    : gilles.claus@ires.in2p3.fr
...
SInt32 EFRIO__MI26_FJtagLoadFile ( char* FileName ) {
  ...
  msg (( MSG_OUT, "### EFRIO__MI26_FJtagLoadFile (FileName=%s)", FileName ));
  ...
  if ( VJtag.IsBound () ) {
    ...
    OleCheck( VRetCode = VJtag.MasterConfSetDevNum ( 0, &VStatus ) );
    
    if ( VRetCode != S_OK ) {
      CoUninitialize();
      err_retfail ( -1, (ERR_OUT,"Set JTAG DevNum 0 failed !" ) );
    }    
...
SInt32 EFRIO__MI26_FJtagReset ( ) {
  ...
  msg (( MSG_OUT, "### EFRIO__MI26_FJtagReset ()" ));
...
SInt32 EFRIO__FJtagLoadChip ( ) {
  ...
  msg (( MSG_OUT, "### EFRIO__FJtagLoadChip () - ParMapsName=%d", VPtRun->ParMapsName ));
...
SInt32 EFRIO__MI26_FJtagLoadChip ( ) {
  ...
  msg (( MSG_OUT, "### EFRIO__MI26_FJtagLoadChip ( ))" ));
...
SInt32 EFRIO__FJtagResetAndLoadChip ( ) {
  ...
  msg (( MSG_OUT, "### EFRIO__FJtagResetAndLoadChip ()" ));
  ...
  msg (( MSG_OUT, "Reset & Reload JTAG => JTAG configuration file = %s", VPtRun->ParJtagFileName ));
  ...
  if ( VRet < 0 ) {
    err_error (( ERR_OUT, "EFRIO__FJtagReset () has failed !" ));
    return (VRet);
  }
  ...
  if ( VRet < 0 ) {
    err_error (( ERR_OUT, "EFRIO__FJtagLoadChip () has failed !" ));
    return (VRet);
  }
...
SInt32 EFRIO__FJtagStartChip ( ) {
  ...
  msg (( MSG_OUT, "### EFRIO__FJtagStartChip ()" ));
...
SInt32 EFRIO__MI26_FJtagStartChip ( ) {
  ...
  msg (( MSG_OUT, "### EFRIO__MI26_FJtagStartChip ()" ));
...
SInt32 EFRIO__FHwStartChip ( SInt32 SpareS32Par ) {
  ...
  msg (( MSG_OUT, "### EFRIO__FHwStartChip (SpareS32Par=%d)", SpareS32Par ));
  
  switch ( VPtRun->ParMapsName ) {
    
    case ASIC__MI26 : {
      msg (( MSG_OUT, "EFRIO__FHwStartChip => Mi26 HW Start" ));
      VRet = EFRIO__MI26_FHwStartChip ( SpareS32Par );
      break; }
    
    case ASIC__ULT1 : {
      msg (( MSG_OUT, "EFRIO__FHwStartChip => ULT1 HW Start" ));
      VRet = EFRIO__ULT1_FHwStartChip ( SpareS32Par );
      break; }
    
    default : {
      msg (( MSG_OUT, "EFRIO__FHwStartChip => HW Start request - But unknown chip !" ));
      err_retfail ( -1, (ERR_OUT,"Unknown MAPS name = %d", VPtRun->ParMapsName) );
      break; }
...
SInt32 EFRIO__MI26_FHwStartChip ( SInt32 SpareS32Par ) {
  ...
  msg (( MSG_OUT, "### EFRIO__MI26_FHwStartChip - a - (Par=%d)", SpareS32Par ));
  
  // Start = D6, Speak = D7*
  
  #ifdef EFRIO__INCLUDE_PARA_PORT
  
  msg (( MSG_OUT, "### EFRIO__MI26_FHwStartChip - b - (Par=%d)", SpareS32Par ));
...
Rev       : 19/05/2011
          ...
          :
          : 30/10/2013
          : Idea following bug fix of EFRIO__FTestOnDataResetErrCnt () on 30/10/2013
          : it may be not needed, but I prefer to implement it.
          : => Update of "MapsNb fields" regardless of Test Start / Stop test 
          : - Update VPtTestCont->ParMapsNb with VPtRunCont->ParMi26Nb
          : - Update VPtTestCont->ParMapsNbToTest with MapsNbToTest
          :
          : 27/01/2014
          : - Param Start replaced by StartAndTestType in order to select a type of test
          : but with only one GUI control => "Start" button replaced by a list box
          : -- Old field EFRIO__TTestOnDataCont.ParModeEnable set to 1 if StartAndTestType <> 0
          : -- New field EFRIO__TTestOnDataCont.ParTestType = StartAndTestType
          :         
Doc date  : /2011
Author    : Gilles CLAUS
E-mail    : gilles.claus@ires.in2p3.fr
...
SInt32 EFRIO__FTestOnDataStartStop ( SInt8 StartAndTestType, SInt8 MapsNbToTest, SInt8 PrintLvl ) {
  ...
  VPtTestCont->ParMapsNb       = VPtRunCont->ParMi26Nb;  // 30/10/13
  VPtTestCont->ParMapsNbToTest = MapsNbToTest;           // 30/10/13
  VPtTestCont->ParTestType     = StartAndTestType;       // 27/01/14

  // Stop and exit

  if ( StartAndTestType == 0 ) {
...
/* DOC_FUNC_BEGIN */
/* ===================================================================================
Prototype : SInt32 EFRIO__FTestOnDataResetErrCnt ()
          :
Goal      :
          :
Inputs    : None
...
Date      : 29/04/2011
Rev       : 29/05/2013
          : Since 29/05/13 it also reads JTAG reference values from JTAG application.
          : The sequence is the following one
          : 1 - Prints status (ref values + errors counts) in log file
          : 2 - Reset errors counter
          : 3 - Reads JTAG references
          : 4 - Prints status (ref values + errors counts) in log file
          :
          : As switch "Enable Data test" is now forced to 1, therefore JTAG reference are read
          : only on first run. The modification done in "Reset data errors counter " is the
          : easiest way to update JTAG reference values at the beginning of each run.
          :
          : 30/10/2013 - Bug fix
          : - Update VPtTestCont->ParMapsNb with VPtRunCont->ParMi26Nb
          : - Otherwise change in MAPS nb from run ctrl is not updated, only done on first run
          :
          :
Doc date  : /2011
Author    : Gilles CLAUS
...
SInt32 EFRIO__FTestOnDataResetErrCnt () {
  ...
  for ( ViMaps=0; ViMaps < VPtTestCont->ParMapsNb; ViMaps++ ) {
    ...
    VPtTestCont->ResAMapsMatrixErrCnt[ViMaps]     = 0; // Mimosa 26 matrix
    
    VPtTestCont->ResAMapsMatrixHitCnt[ViMaps]     = 0; // Mimosa 26 hits cnt
  }
  ...
  // Rev : 29/05/13
  // Get JTAG reference values from JTAG application
  ...
  EFRIO__FPrintTestOnDataCont ( "Reset errors count done - Reference values after JTAG reading" );
...
/* DOC_FUNC_BEGIN */
/* ===================================================================================
Prototype : SInt32 EFRIO__FRestartDaqWaitOnDutDaq ( SInt32 TimeBeforeWaitMs, SInt8 WaitOnDutDaq, SInt32 TimeToWaitnDutDaqMs )
:
Goal      : 
:
Inputs    : 
...
SInt32 EFRIO__FRestartDaqWaitOnDutDaq ( SInt32 TimeBeforeWaitMs, SInt8 WaitOnDutDaq, SInt32 TimeToWaitOnDutDaqMs ) {
  
  SInt32            ViWaitLoop;
  SInt32            VWaitLoopNb;
  SInt32            VLoopPeriodMs;
  EFRIO__TContext*  VPtCont  = &EFRIO__VGContext;
    

  msg (( MSG_OUT, "Wait on DUT DAQ begin" ));
  
  VLoopPeriodMs = 30;

  // Set "Start/Restart" line to 0
  
  PPO_FOutD4 ( 0 /* Id */, 0 /* State */ );
  
  // Time before wait on DUT DAQ answer
  
  Application->ProcessMessages ();
  Sleep ( TimeBeforeWaitMs );
  
  
  // Wait on DUT DAQ acknowledge
  
  if ( WaitOnDutDaq ) {
    
    VWaitLoopNb = TimeToWaitOnDutDaqMs / VLoopPeriodMs;
  
    // Wait loop on DUT DAQ ack
  
    for ( ViWaitLoop=0; ViWaitLoop < VWaitLoopNb; ViWaitLoop++) {
      
      // msg (( MSG_OUT, "Wait on DUT DAQ : Loop %d ", ViWaitLoop ));
      
      Application->ProcessMessages ();      
      Sleep ( VLoopPeriodMs );
      
      if ( PPO_FInBusy ( 0 /* Id */ ) == 0 ) {
        PPO_FOutD4 ( 0 /* Id */, 1 /* State */ ); // Reset "Start/Restart" line
        msg (( MSG_OUT, "Wait on DUT DAQ end => ACQ RECEIVED :-)" ));
        return (1);
      }
    ...
    PPO_FOutD4 ( 0 /* Id */, 1 /* State */ ); // Reset "Start/Restart" line
    msg (( MSG_OUT, "Wait on DUT DAQ end => NO ACQ RECEIVED !" ));
    return (0);
    ...
    // No test on DUT ack => Return 1 after " TimeBeforeWaitMs "
    
    PPO_FOutD4 ( 0 /* Id */, 1 /* State */ ); // Reset "Start/Restart" line
    msg (( MSG_OUT, "Wait on DUT DAQ end => NO WAIT ON DUT DAQ" ));
    return (1);
  }
