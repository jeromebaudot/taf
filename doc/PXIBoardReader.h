...
class PXIEvent : public TObject {
 ...
 ~PXIEvent();
  ...
  vector<PXIPixel> *GetPixels()                  { return ListOfPixels;}
...
class PXIBoardReader : public TObject {
 ...
 private:
  ...
  PXIBoardReader() {;}
  PXIBoardReader( int boardNumber, char *configFileName, int triggerMode, int endian=0);
  ~PXIBoardReader();
