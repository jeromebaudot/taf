...
/* DOC_FUNC_END */
  ...
  SInt32 FIL_FGetAppRunDir ( char* RunDirStr ) {
    ...
    err_retfailnull ( RunDirStr, (ERR_OUT,"RunDirStr == NULL") );
...
SInt32  FIL__TCBinFile :: PubFSeqRead  ( void* DestPtr, SInt32 MaxDestSz, SInt32 DataSzToRead ) {
  ...
  if ( VBlocNbRead != 1 ) {
    err_retfail ( -1, (ERR_OUT,"Bloc read failed ! => System : %s", _strerror ("") ) );
  }
...
SInt32 FIL__TCStreamFile :: PubFPrintInfFile () {
  ...
  msg (( MSG_OUT, "ABlocInf[0].SpareW32Info = %d  ", ProPtRecInfFile->ABlocInf[0].SpareW32Info ));
  msg (( MSG_OUT, "-------------------------" ));
  msg (( MSG_OUT, "ABlocInf[1].Offset       = %Lu", ProPtRecInfFile->ABlocInf[1].Offset ));
  msg (( MSG_OUT, "ABlocInf[1].Sz           = %d  ", ProPtRecInfFile->ABlocInf[1].Sz ));
  msg (( MSG_OUT, "ABlocInf[1].SpareW32Info = %d  ", ProPtRecInfFile->ABlocInf[1].SpareW32Info ));
  msg (( MSG_OUT, "-------------------------" ));
  
  if ( ProPtRecInfFile->FixedBlocSz == 0 ) {
    
    for ( ViBloc=0; ViBloc < ProPtRecInfFile->BlocNb; ViBloc++ ) {
      msg (( MSG_OUT, "Bloc[%.4d] : Offset=%.12Lu - Sz=%d [Bytes]", ViBloc, ProPtRecInfFile->ABlocInf[ViBloc].Offset, ProPtRecInfFile->ABlocInf[ViBloc].Sz ));
    }
...
SInt32 FIL__TCStreamFile :: PubFSeqWrite ( void* PtrData, SInt32 DataSz, SInt32 SpareW32Info ) {
  ...
  if ( ProUseThread == 1 ) {
    ...
    switch ( VBuffFree ) {
      ...
      case WAIT_TIMEOUT : {
        ++ProResWrBlocFailCnt;
        err_retfail ( -1, (ERR_OUT,"Write failed for the %d time -> No buffer available for event %d!", ProResWrBlocFailCnt, SpareW32Info ) );
        break; }
      ...
      case WAIT_OBJECT_0	 : {
        ...
        err_trace (( ERR_OUT, "Thread => Buffer %d filled with Event %d", ProIndexWrBuff, SpareW32Info ));
  ...
  if ( ProParFixedBlocSzMode == 0 ) {
    ...
    ProPtRecInfFile->ABlocInf[ProPtRecInfFile->BlocNb].SpareW32Info = SpareW32Info;
  }
...
SInt32 FIL__TCStreamFile :: PubFGotoBloc  ( SInt32 BlocNo ) {
  ...
  UInt32  VRet;
  UInt64  VOffsetW64;
  SInt32* VPtOffsetLow32;
  SInt32* VPtOffsetHigh32;
  LPVOID  VMsgBuf;
  ...
  VPtOffsetLow32  = ((SInt32*) &VOffsetW64);
  VPtOffsetHigh32 = ((SInt32*) &VOffsetW64) + 1;
  ...
  VRet = SetFilePointer (
    ...
    *VPtOffsetLow32,	      // number of bytes to move file pointer
    ...
    VPtOffsetHigh32,	      // address of high-order word of distance to move
...
SInt32 FIL__TCStreamFile :: PubFBlocRead  ( SInt32 BlocNo, void* DestPtr, SInt32 MaxDestSz, SInt32* PtSpareW32Info ) {
  ...
  if ( ProParFixedBlocSzMode == 0 ) {
    ...
    if ( PtSpareW32Info != NULL ) {
      *PtSpareW32Info = ProPtRecInfFile->ABlocInf[BlocNo].SpareW32Info;
    }
  }
