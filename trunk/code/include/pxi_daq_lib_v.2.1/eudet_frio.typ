...
typedef struct {
  ...
  EFRIO__TFrame* PtFrame;                      // If data ParDataTransferMode = EUDET 1,2,3  => Acquisition frames buffer
    ...
    #error NOT allowed => Only one APP_DLL__TELESCOPE_MI26_MI28_DATA_FORMAT_CC2_... directive at a time !
  #endif
...
typedef struct {
  ...
  char   InfConfFileName[GLB_FILE_PATH_SZ];    // Run configuration file ( save EFRIO__TRunCont to disk ) name built form ParRunNo, ParDestDir
  char   InfDataFileName[GLB_FILE_PATH_SZ];    // Run data file name built from ParRunNo, ParFileNamePrefix, ParDestDir
  char   InfIndexFileName[GLB_FILE_PATH_SZ];   // Run index file name built from ParRunNo, ParFileNamePrefix, ParDestDir - 10/07/2012
...
/* Monitoring record                              */
/* ---------------------------------------------- */
/* This record contains tests on data conf / res  */
/* ---------------------------------------------- */
/* Date      : 29/04/2011                         */
/* Doc date  : 29/04/2011                         */
/* Author    : Gilles CLAUS                       */
...
typedef struct {
  ...
  SInt8  ParTestType;     // 
  SInt8  ParModeEnable;   // Enable test
  SInt8  ParResetCnt;     // Reset all error counters
  SInt8  ParPrintLvl;     // Errors print level
  SInt8  ParMapsNb;       // Nb of MAPS configured in JTAG
  SInt8  ParMapsNbToTest; // Number of MAPS on which the test is performed
                          // MAPS no 0 to ParMapsNbToTest - 1
  
  // Reference / expected values
  
  UInt32 ParAMapsHeaderRef[EFRIO__MAX_ASIC_NB];          // Mimosa 26/28 header field
  UInt32 ParAMapsTrailerRef[EFRIO__MAX_ASIC_NB];         // Mimosa 26/28 trailer field

  ULT1__TRegDiscriW32   ParUlt1ALinePat0[EFRIO__MAX_ASIC_NB];  // Mimosa 28 line patten
  ULT1__TRegDiscriW32   ParUlt1ALinePat1[EFRIO__MAX_ASIC_NB];
  
  SInt16 ParAMatPatternLineNb [EFRIO__MAX_ASIC_NB];      // Mimosa 26/28 Max line number in emulated matrix
  

  UInt32 InfMapsFrameCntRef;                             // Mimosa 26 frame counter field
  
  ULT1__TMatDiscriBit InfUlt1AMatBitFromJtag[EFRIO__MAX_ASIC_NB];
  ULT1__TMatDiscriW32 InfUlt1TmpMatW32;
    
  // Current values get from DAQ
  
  UInt32 ResAMapsHeaderVal[EFRIO__MAX_ASIC_NB];          // Mimosa 26 header field
  UInt32 ResAMapsFrameCntVal[EFRIO__MAX_ASIC_NB];        // Mimosa 26 frame counter field
  UInt16 ResAMapsDataLengthVal[EFRIO__MAX_ASIC_NB];      // Mimosa 26 data length in BYTES -> It's final result NOT the DataLength FIELD from data stream
  UInt32 ResAMapsTrailerVal[EFRIO__MAX_ASIC_NB];         // Mimosa 26 trailer field
  
  ULT1__TZsFFrame ResUlt1AZsFFrame[EFRIO__MAX_ASIC_NB];
    
  ULT1__TMatDiscriBit ResUlt1AMatBitFromDaq[EFRIO__MAX_ASIC_NB]; // Mimosa 28 matrix from DAQ
  
  
  // Errors counters

  SInt32 ResAMapsErrCnt[EFRIO__MAX_ASIC_NB];              // Err counter / MAPS = sum errors on header, fc, trailer
  
  SInt32 ResAMapsHeaderErrCnt[EFRIO__MAX_ASIC_NB];        // Mimosa 26 header field
  SInt32 ResAMapsFrameCntErrCnt[EFRIO__MAX_ASIC_NB];      // Mimosa 26 frame counter field
  SInt32 ResAMapsDataLengthErrCnt[EFRIO__MAX_ASIC_NB];    // Mimosa 26 data length in BYTES -> It's final result NOT the DataLength FIELD from data stream
  SInt32 ResAMapsTrailerErrCnt[EFRIO__MAX_ASIC_NB];       // Mimosa 26 trailer field
  SInt32 ResAMapsMatrixErrCnt[EFRIO__MAX_ASIC_NB];        // Mimosa 26 matrix
  
  
  SInt32 ResTotErrCnt;        // Total errors counter = sum of N MAPS
  SInt32 ResFrameNbWithErr;   // Number of frames with at least one error
  SInt8  ResErrOnCurrentAcq;  // Indicates errors on current acq - 25/04/2013
                              // 0 => None
                              // 1 => Errors

  SInt32 ResAMapsMatrixHitCnt[EFRIO__MAX_ASIC_NB];        // 06/02/14 - Mimosa 28 hits cnt
  

} EFRIO__TTestOnDataCont; 
...
typedef struct {
                                                           ...
                                                           // Add on 11/05/2011
                                                           // Lib version, eg : 1.2  
  SInt8 VersionMajor;                                      // Lib version major : version 1.2 => 1
  SInt8 VersionMinor;                                      // Lib version minor : version 1.2 => 2
  
                                                           // Add on 11/05/2011
  char  VersionCmt[GLB_CMT_SZ];                            // Commetn about lib version = content of lib
  ...
  EFRIO__TTestOnDataCont TestOnDataCont;                   // Test on data context
  ...
  // Used to solve FW trigger bug ( Ultimate => extra channel info shifted by two frames )
  
  // For each frame Id stored in AAAcqFrameWithTrigList
  // we store in the trigger nb in FwTrigBugAAAcqFrameWithTrigTrigNb
  
  SInt16              FwTrigBugAAAcqFrameWithTrigTrigNb[1][EFRIO__MAX_FRAME_NB_PER_ACQ];
  
  // Is set to 1 if this frame contains the trigger list of frame No - 2
  
  SInt16              FwTrigBugAAAcqFrameWithTrigIsTrigListFrMinus2[1][EFRIO__MAX_FRAME_NB_PER_ACQ + 10];
