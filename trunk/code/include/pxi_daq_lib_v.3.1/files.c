...
/*******************************************************************************
File      : x:\lib\com\files\files.c
Goal      : Functions of files library
Prj date  : XX/XX/2003
File date : XX/XX/2003
          :
Rev       : 11/01/2012
          : - Plume BT bug fix in TCStreamFile(ProParBlocSz -> ProAParBlocSz)
          : 12/01/2012
          : - Robustness improvment by tagging (add - to size info) buffers when write
          : on RAID has failed, these blocs will be removed from blocs index table
          : - Save two index files :
          :   *.inf     = normal without info on blocs not saved
          :   *.all.inf = default with all blocs info
          :       
Doc date  : 
Author    : Gilles CLAUS
...
SInt32 FIL_FDirOutFile ( char* SrcDir, char* DestDirFile ) {
  ...
  #else
    err_retfail ( -1, (ERR_OUT,"Function not handled under VC++") );
  #endif
...
SInt32 FIL_FRmDirFiles ( char* SrcDir ) {
  ...
  #else
    err_retfail ( -1, (ERR_OUT,"FIL_FRmDirFiles (SrcDir=%s) IS NOT available under VC++ !", SrcDir) );
  #endif
...
SInt32 FIL_FRmDir ( char* SrcDir ) {
  ...
  #else
    err_retfail ( -1, (ERR_OUT,"Function handled only under VC++") );
  #endif
...
SInt32 FIL_FMkDir ( char* SrcDir ) {
  ...
  char VStrCmd[GLB_FILE_PATH_SZ]; // Debug 02/06/2007 move storage class static to auto
    ...
    #else
      err_retfail ( -1, (ERR_OUT,"Function NOT available under VC++ !") );
    #endif
...
SInt32 FIL_FTextFileToFloatVectors ( char* SrcFile, char DataSep, float** DestVectors, SInt16 VectorsNb, SInt32 EltNbPerVector, char* HeaderLine, char** AVectorsNames, char* StrVectorsNames  ) {
  ...
  SInt32 ViVector;
  SInt32 ViLine;
  SInt32 VRet = 1; // Antonin Maire, proper initialisation of the return variable, 16 oct 2014
  char   VStrCR[2];
  char   VDataSep;
  ...
  char *c = 0x0;
  c = fgets( HeaderLine, 255, VPf );
  if( c == 0x0 ) VRet = 0;
  c = 0x0;
    // change Antonin Maire, 16 oct 2014 (cancel Jérôme Baudot's commit 1027)
    // Was before : VRet = (SInt32) fgets( HeaderLine, 255, VPf );
    // Pb : Unlike fscanf, fgets return a char* and not an int...
    //    -> http://www.cplusplus.com/reference/cstdio/fgets/
    // gcc compilation does not like the cast char* to SInt32 VRet.
#endif
  ...
  c = fgets( StrVectorsNames, 255, VPf ); 
  // change Antonin Maire, 16 oct 2014
  if( c == 0x0 ) VRet = 0;
  c = 0x0;
    // change Antonin Maire, 16 oct 2014 (cancel Jérôme Baudot's commit 1027)
    // Was before :  VRet = (SInt32) fgets( StrVectorsNames, 255, VPf );
    // Pb : Unlike fscanf, fgets return a char* and not an int...
    //    -> http://www.cplusplus.com/reference/cstdio/fgets/
    // gcc compilation does not like the cast char* to SInt32 VRet.
...
/* DOC_FUNC_END */
  ...
  #ifndef FILE__NO_GET_APP_RUN_DIR
...
SInt32  FIL__TCBinFile :: PubFSeqRead  ( void* DestPtr, SInt32 MaxDestSz, SInt32 DataSzToRead ) {
  ...
  ProCurRdSz = DataSzToRead;
    ...
    #endif
  #endif
  //msg  (( MSG_OUT, "sizeof (ProPtFile) = %d", sizeof (ProPtFile) ));
  //msg  (( MSG_OUT, "ProCurRdSz = %d", ProCurRdSz ));
...
FIL__TCStreamFile :: ~FIL__TCStreamFile () {
  ...
  // Free info record cpy
  
  if ( ProPtRecInfFileCpy != NULL ) {
    free ( ProPtRecInfFileCpy );
  }
...
Rev       : 09/05/2010
          ...
          : 11/01/2012
          : - "Plume BT bug" fix, using new field ProAParBlocSz instead of ProABlocSz
          : 12/01/2012
          : - Robustness improvment by tagging (add - to size info) buffers when write
          : on RAID has failed, these blocs will be removed from blocs index table
          :
...
DWORD WINAPI FIL__TCStreamFile_FThread (  LPVOID lpParam ) {
  ...
  static SInt32 VDbgSaveCnt = 0;
  
  #ifdef FIL__DEBUG
    EFRIO__TFrame* VDbgPtFrame;
  #endif
  ...
  while (1) {
    ...
    switch ( VBuffFull ) {
      ...
      case WAIT_OBJECT_0 : {
        ...
        VNbBytesToWrite = VPtObj->ProAParBlocSz[VPtObj->ProIndexRdBuff]; // 11/01/2012 Plume BT bug fix
        
        // Emulates error during write on RAID 1 each 100 access
        // It's DISABLED now
      
        ++VDbgSaveCnt;
        
        if ( 0 /* (VDbgSaveCnt % 100) == 0 */ ) { // 
          VNbBytesWritten = 0;
        }
        ...
        if ( VNbBytesWritten != VNbBytesToWrite ) {
          ...
          // 12/01/2012
          // - Tag an error maker on the bloc = set its size to negative value
          // - This tag will be used by ProFUpdateIndexTable () in PubFClose () to remove blocs not saved from blocs table
          
          *(VPtObj->ProAPtBlocSz[VPtObj->ProIndexRdBuff]) = -(*(VPtObj->ProAPtBlocSz[VPtObj->ProIndexRdBuff]));
          
          // Log error
          
          sprintf ( VStrMsg, "ThreadDisk  => Writing buff %d error !", VPtObj->ProIndexRdBuff );
          VPtObj->PubFPrintMsg ( VStrMsg, FIL__TCStreamFile_LOG_ERR_ERROR );
        }
        
        else {
          sprintf ( VStrMsg, "ThreadDisk  => Buffer %d save to disk", VPtObj->ProIndexRdBuff );
          VPtObj->PubFPrintMsg ( VStrMsg, FIL__TCStreamFile_LOG_ERR_TRACE );
        }
...
SInt32 FIL__TCStreamFile :: PubFBegin ( char* ErrLogFile, SInt8 EnableErrLog, SInt8 ErrLogLvl, SInt32 DiskBlocSz ) {
  ...
  // Parameter used for multithreading data saving mode 

  for ( ViBuff=0; ViBuff < FIL__TCStreamFile_MAX_BUFF_NB; ViBuff++ ) {
    ProAParBlocSz[FIL__TCStreamFile_MAX_BUFF_NB] = 0;
  }
...
Prototype :
Goal      :
Inputs    : Msg      - The message string
          : MsgLevel - It can be
          :  - FIL__TCStreamFile_LOG_GENERAL_MSG
          :  - FIL__TCStreamFile_LOG_ERR_TRACE    
          :  - FIL__TCStreamFile_LOG_ERR_WARNING  
          :  - FIL__TCStreamFile_LOG_ERR_ERROR    
          :
Ouputs    :
Globals   :
Remark    :
...
void FIL__TCStreamFile :: PubFPrintMsg ( char* Msg, SInt8 MsgLevel ) {
  ...
  switch ( MsgLevel ) {
    ...
    case FIL__TCStreamFile_LOG_GENERAL_MSG : {
      msg (( MSG_OUT, "%s", Msg  ));
      break; }
    
    case FIL__TCStreamFile_LOG_ERR_TRACE : {
      err_trace (( ERR_OUT, "%s", Msg ));
      break; }

    case FIL__TCStreamFile_LOG_ERR_WARNING : {
      err_warning (( ERR_OUT, "%s", Msg ));
      break; }

    case FIL__TCStreamFile_LOG_ERR_ERROR : {
...
Date      : 01/05/2010
          :
Rev       : 11/01/2012
          : - "Plume BT bug" fix, add cpy of RecInfFile => ProPtRecInfFileCpy
          :
Doc date  : 01/05/2010
Author    : Gilles CLAUS
E-mail    : gilles.claus@ires.in2p3.fr
...
SInt32 FIL__TCStreamFile :: PubFConf ( FIL__TCStreamFile* PtMyself, SInt8 UseThread, char* DataFile, SInt8 RWBMode, SInt8 FixedBlocSzMode, SInt32 MaxBlocSz, SInt32 BlocSz, SInt8 FlushAfterWrite, SInt8 MeasTime ) {
...
#ifndef CC_UNIX
  #ifndef FILE__NO_DATE_TIME_HANDLING
  ...
  sprintf ( ProInfFileNameAll, "%s.all.inf", ProParDataFile );
  ...
  err_warning (( ERR_OUT, "ProRecInfSz=%d ", ProRecInfSz )); // added, JB 2014/10/16
  ...
  // Add cpy of RecInfFile on 11/01/2012

  ProPtRecInfFileCpy = (FIL__TCStreamFile_TRecInfFile*) malloc ( ProRecInfSz );
  
  err_retnull ( ProPtRecInfFileCpy, (ERR_OUT,"Allocation of info record cpy failed !") );
  
  memset ( ProPtRecInfFileCpy, 0, ProRecInfSz );
...
Ouputs    :
Globals   :
Remark    : Prints record passed as parameter, not the one of the class like PubFPrintInfFile ()
          : It's also possible to add a comment and a "tag string" at the beginning of each line.
Level     :
Date      : 13/01/2012
Doc date  : 13/01/2012
Author    : Gilles CLAUS
...
SInt32 FIL__TCStreamFile :: PubFPrintInfFileRec ( FIL__TCStreamFile_TRecInfFile* Pt, char* Cmt, char* TagStr ) {
  ...
  msg (( MSG_OUT, "$" ));
  msg (( MSG_OUT, "-------------------------" ));
  msg (( MSG_OUT, " Info file record - %s", Cmt ));
  msg (( MSG_OUT, "-------------------------" ));
  msg (( MSG_OUT, "%s Version     = %d", TagStr, Pt->Version ));
  msg (( MSG_OUT, "-------------------------" ));
  msg (( MSG_OUT, "%s Date create = %s", TagStr, TIME__FDateL2Str ( Pt->DateCreate, NULL, 0 ) ));
  msg (( MSG_OUT, "%s Time create = %s", TagStr, TIME__FTime2Str  ( Pt->TimeCreate, NULL, 0 ) ));
  msg (( MSG_OUT, "%s Date close  = %s", TagStr, TIME__FDateL2Str ( Pt->DateClose , NULL, 0 ) ));
  msg (( MSG_OUT, "%s Time close  = %s", TagStr, TIME__FTime2Str  ( Pt->TimeClose , NULL, 0 ) ));
  msg (( MSG_OUT, "-------------------------" ));
  msg (( MSG_OUT, "%s FixedBlocSz = %d", TagStr, Pt->FixedBlocSz ));
  msg (( MSG_OUT, "%s MaxBlocSz   = %d", TagStr, Pt->MaxBlocSz   ));
  msg (( MSG_OUT, "%s BlocSz      = %d", TagStr, Pt->BlocSz      ));
  msg (( MSG_OUT, "%s BlocNb      = %d", TagStr, Pt->BlocNb      ));
  msg (( MSG_OUT, "-------------------------" ));
  msg (( MSG_OUT, "%s ABlocInf[0].Offset             = %Lu" , TagStr, Pt->ABlocInf[0].Offset             ));
  msg (( MSG_OUT, "%s ABlocInf[0].Sz                 = %d  ", TagStr, Pt->ABlocInf[0].Sz                 ));
  msg (( MSG_OUT, "%s ABlocInf[0].SpareW32InfoFormat = %d  ", TagStr, Pt->ABlocInf[0].SpareW32InfoFormat ));
  msg (( MSG_OUT, "%s ABlocInf[0].SpareW32InfoNb     = %d  ", TagStr, Pt->ABlocInf[0].SpareW32InfoNb     ));
  
  for ( ViSpareW32Info=0; ViSpareW32Info <  Pt->ABlocInf[0].SpareW32InfoNb; ViSpareW32Info++ ) {
    msg (( MSG_OUT, "%s ABlocInf[0].ASpareW32Info[%.2d] = %d  ", TagStr, ViSpareW32Info, Pt->ABlocInf[0].ASpareW32Info[ViSpareW32Info] ));
  }
  
  
  if ( Pt->FixedBlocSz == 0 ) {
    
    for ( ViBloc=0; ViBloc < 200 /* Pt->BlocNb */; ViBloc++ ) {
      msg (( MSG_OUT, "%s Bloc[%.4d] : Offset=%.12Lu - Sz=%.8d [Bytes] - ASpare [0]=%.4d [1]=%.4d [2]=%.4d [3]=%.4d", TagStr, ViBloc, Pt->ABlocInf[ViBloc].Offset, Pt->ABlocInf[ViBloc].Sz, Pt->ABlocInf[ViBloc].ASpareW32Info[0], Pt->ABlocInf[ViBloc].ASpareW32Info[1], Pt->ABlocInf[ViBloc].ASpareW32Info[2], Pt->ABlocInf[ViBloc].ASpareW32Info[3] ));
    }
...
Date      : 01/05/2010
Rev       : 13/01/2012
          : - Add handling of flag ProFileHasBeenClosed, needed for Robustness
          : improvment of data saving on RAID implemented on 12/01/2012.
          :
Doc date  : 01/05/2010
Author    : Gilles CLAUS
E-mail    : gilles.claus@ires.in2p3.fr
...
SInt32 FIL__TCStreamFile :: PubFCreate () {
  ...
  if ( ProParRWBMode != FIL__TCBinFile_RWB_MODE_WRITE ) {
    err_retfail ( -1, (ERR_OUT,"Abort : Can't create a file when RWB mode IS NOT Write !") );
  }
...
Rev       : 09/05/2010
          ...
          :
          : 11/01/2012
          : - Plume BT bug fix (corrupted data while saving in multithreading mode)
          : - Bloc size saving per bloc was not implemented, it was done for one bloc
          : - Implementation done via ProAParBlocSz array.
          :
          : 12/01/2012
          : - Robustness improvment by tagging (add - to size info) buffers when write
          : on RAID has failed, these blocs will be removed from blocs index table
...
SInt32 FIL__TCStreamFile :: PubFSeqWrite ( void* PtrData, SInt32 DataSz, SInt32 DbgCallPar, SInt32 SpareW32InfoFormat, SInt32* PtSpareW32Info, SInt16 SpareW32InfoNb ) {
  ...
  #ifdef FIL__DEBUG
    EFRIO__TFrame* VDbgPtFrame;
    EFRIO__TFileSpareW32Info* VDbgPtSpareW32Info;
  #endif
  
  
  
  #ifdef FIL__DEBUG
    VDbgPtFrame        = (EFRIO__TFrame*) PtrData;
    VDbgPtSpareW32Info = (EFRIO__TFileSpareW32Info*) PtSpareW32Info;
  #endif
  ...
  if ( ProParFixedBlocSzMode == 0 ) {
    ...
    if ( ProPtRecInfFile->BlocNb >= FIL__TCStreamFile_MAX_BLOC_NB_IN_VAR_BLOC_SZ_MODE ) {
      err_retfail ( -2, (ERR_OUT,"$$$ Max bloc nb = %d reached in variable bloc sz mode !", FIL__TCStreamFile_MAX_BLOC_NB_IN_VAR_BLOC_SZ_MODE) );
    }
  ...
  if ( ProUseThread == 1 ) {
    ...
    switch ( VBuffFree ) {
      ...
      case WAIT_OBJECT_0	 : {
        ...
        // Update index file table => Code moved here on 12/01/2012
        ...
        ProAPtBlocSz[ProIndexWrBuff]  = &ProPtRecInfFile->ABlocInf[ProPtRecInfFile->BlocNb].Sz; // Store ptr on bloc size
        ...
        ProAParBlocSz[ProIndexWrBuff] = ProParBlocSz; // 11/01/2012 Plume BT bug fix
  ...
  } // END "else write now data on disk"
  
  
/* Modified on 12/01/2012
   => This part can not be common for non multithreading and multithreading data saving mode
   => Because it must be executed before before data saving code in case of multithreading and after otherwise
   => Therefore, it has been dupplicated in the part of source code which handles each mode
...
SInt32 FIL__TCStreamFile :: PubFFlush () {
  ...
  if ( ProFileHasBeenClosed == 1 ) {
    err_warning (( ERR_OUT, "File has already been closed !" ));
    return (0);
  }
...
/*******************************************************************************
Prototype :
Goal      : Remove the blocs which have not been saved (write on RAID has failed)
          : from blocs table.
Inputs    :
Ouputs    :
Globals   :
Remark    : Robustness improvment by tagging (add - to size info) buffers when write
          : on RAID has failed (FIL__TCStreamFile_FThread), these blocs will be removed
          : from blocs index table.
          :
Level     :
Date      : 12/01/2012
          :
...
SInt32 FIL__TCStreamFile :: ProFUpdateIndexTable () {
  
  SInt32 VRet;
  SInt32 ViSrcBloc;
  SInt32 ViDestBloc;
  SInt32 VSrcTotBlocsNb;
  SInt32 VDestGoodBlocsNb;
  SInt32 VOffsetCorrection;
  
   
  // Copy normal table in temporary record
  
  memcpy ( ProPtRecInfFileCpy, ProPtRecInfFile, ProRecInfSz );
  
  // Reset normal table
  
  memset ( ProPtRecInfFile, 0, ProRecInfSz );
  
  // Restor fields - We want to reset bloc list not "header fields"

  ProPtRecInfFile->Version     = ProPtRecInfFileCpy->Version;
  ProPtRecInfFile->DateCreate  = ProPtRecInfFileCpy->DateCreate;
  ProPtRecInfFile->TimeCreate  = ProPtRecInfFileCpy->TimeCreate;
  ProPtRecInfFile->DateClose   = ProPtRecInfFileCpy->DateClose;
  ProPtRecInfFile->TimeClose   = ProPtRecInfFileCpy->TimeClose;
  ProPtRecInfFile->FixedBlocSz = ProPtRecInfFileCpy->FixedBlocSz;
  ProPtRecInfFile->MaxBlocSz   = ProPtRecInfFileCpy->MaxBlocSz;
  ProPtRecInfFile->BlocSz      = ProPtRecInfFileCpy->BlocSz;
  ProPtRecInfFile->BlocNb      = ProPtRecInfFileCpy->BlocNb;
    

  // Rebuild normal table with blocs non corrupted
  
  VOffsetCorrection = 0;
  VSrcTotBlocsNb    = ProPtRecInfFileCpy->BlocNb;
  VDestGoodBlocsNb  = 0;
  ViDestBloc        = 0;
  
  for ( ViSrcBloc=0; ViSrcBloc < VSrcTotBlocsNb; ViSrcBloc++ ) {
  
    if ( ProPtRecInfFileCpy->ABlocInf[ViSrcBloc].Sz > 0 ) {
      ProPtRecInfFile->ABlocInf[ViDestBloc]        = ProPtRecInfFileCpy->ABlocInf[ViSrcBloc];
      ProPtRecInfFile->ABlocInf[ViDestBloc].Offset = ProPtRecInfFileCpy->ABlocInf[ViSrcBloc].Offset + VOffsetCorrection;
      ++ViDestBloc;
    }
    
    else {
      VOffsetCorrection = VOffsetCorrection + ProPtRecInfFileCpy->ABlocInf[ViSrcBloc].Sz; // Add negative size
    }
    
  } // End for
  
  VDestGoodBlocsNb = ViDestBloc;
  
  // Overwrite normal table blocs nb with good blocs nb

  ProPtRecInfFile->BlocNb = VDestGoodBlocsNb;
...
Rev       : 12/01/2012
          : - Robustness improvment by tagging (add - to size info) buffers when write
          : on RAID has failed, these blocs will be removed from blocs index table
          : - Call ProFUpdateIndexTable ()
          : - Save two index files :
          :   *.inf     = normal without info on blocs not saved
          :   *.all.inf = default with all blocs info
          :
          : 13/01/2012
          : - Add flag ProFileHasBeenClosed, in order to avoid multiple calls
          : of this function, because it's NOT allowed with the new handling of
          : index table (suppression of bad blocs).
          :
          :
Doc date  : 01/05/2010
Author    : Gilles CLAUS
...
SInt32 FIL__TCStreamFile :: PubFClose () {
  ...
  ProFUpdateIndexTable ();


  if ( ProReadyToWrite == 1 ) {
  
    // -------------------------------
    // Save updated index file table
    // -------------------------------
    ...
    err_error (( ERR_OUT, "TRACE ProPtRecInfFile = %X", ProPtRecInfFile ));
    ...
    // -------------------------------
    // Save default index file table = with all blocs info
    // -------------------------------
    
    // Create info file
    
    ProPtInfFileAll = fopen ( ProInfFileNameAll, "wb" );
    
    err_retnull ( ProPtInfFileAll, (ERR_OUT,"Create of info file ALL %s failed !", ProInfFileNameAll) );
    
    err_error (( ERR_OUT, "TRACE ProPtRecInfFile = %X", ProPtRecInfFile ));
    
    if ( fwrite ( ProPtRecInfFileCpy, ProRecInfSz, 1, ProPtInfFileAll ) != 1 ) {
      err_retfail ( -1, (ERR_OUT,"Write info file %s failed !", ProInfFileNameAll) );
    }
    
    if ( fclose (ProPtInfFileAll) != 0 ) {
      err_retfail ( -1, (ERR_OUT,"fclose on info file ALL failed => System : %s", strerror ( errno ) ) );
    }
...
SInt32 FIL_FIndexFileDutCreate ( char* FilePath ) {
  ...
  char VFuncName[]            = "FIL_FIndexFileDutCreate";
  FIL__TIndexFileDut* VPtCont = &FIL__VGIndexFileDut;
  SInt32 VRet;
  
  // Normal file
  
  sprintf ( VPtCont->FilePath, "%s", FilePath );
  
  VPtCont->PfDest = fopen ( VPtCont->FilePath, "wb" );
        
  err_retnull ( VPtCont->PfDest, (ERR_OUT,"Destination file %s creation failed => %s", VPtCont->FilePath, _strerror ( "System says :" )) );

  VPtCont->EltCnt = 0;

  fclose (VPtCont->PfDest);
  
#ifdef AFRIO__CREATE_INDEX_FILE_CPY

  // Cpy file   
 
  VPtCont->PfDestCpy = fopen ( FIL__DUT_INDEX_FILE_CPY_NAME, "wb" );
  
  err_retnull ( VPtCont->PfDestCpy, (ERR_OUT,"Destination file %s creation failed => %s", FIL__DUT_INDEX_FILE_CPY_NAME, _strerror ( "System says :" )) );
    
  fclose (VPtCont->PfDestCpy);
...
SInt32 FIL_FIndexFileDutAddEv ( UInt32 EvId, UInt32 EvTag, UInt32 TrigLine, UInt32 TrigFrame, UInt32 TrigNbTot, UInt32 TrigNbAccepted, UInt32 SpareU32 ) {
  ...
  char VFuncName[]            = "FIL_FIndexFileDutAddEv";
  FIL__TIndexFileDut* VPtCont = &FIL__VGIndexFileDut;
  UInt32 VAFields[8];
  SInt32 VRet;
  ...
  VPtCont->PfDest = fopen ( VPtCont->FilePath, "ab" );
  
  err_retnull ( VPtCont->PfDest, (ERR_OUT,"Destination file %s open for append failed => %s", VPtCont->FilePath, _strerror ( "System says :" ) ));
  
  VAFields[0]  = 1;      // Version
  VAFields[1]  = EvId;
  VAFields[2]  = EvTag;
  VAFields[3]  = TrigLine;
  VAFields[4]  = TrigFrame;
  VAFields[5]  = TrigNbTot;
  VAFields[6]  = TrigNbAccepted;
  VAFields[7]  = SpareU32;
  
  VRet = fwrite ( VAFields, 32 /* size = 8 x U32 */, 1 /* Elt nb */, VPtCont->PfDest );
  
  
  if ( VRet != 1 ) {
    err_retfail ( -1, (ERR_OUT,"Error writing record %d => %s", EvId, _strerror ( "System says :" ) ));
  }
  
  VRet = fflush ( VPtCont->PfDest );

  if ( VRet != 0 ) {
    err_retfail ( -1, (ERR_OUT,"Error flush record %d => %s", EvId, _strerror ( "System says :" ) ));
  }

  VRet = fclose ( VPtCont->PfDest );
  
  if ( VRet != 0 ) {
    err_retfail ( -1, (ERR_OUT,"Error fclose record %d => %s", EvId, _strerror ( "System says :" ) ));
  }
  
  ++VPtCont->EltCnt;

  // Cpy file
  // Rq : In case of error => log error BUT don't return < 0 error code because w/r conflict may happen
  // on the cpy index file and must not stop run
  ...
  if ( 0 ) {
    ...
    while (1) {
      ...
      VPtCont->PfDestCpy = fopen ( FIL__DUT_INDEX_FILE_CPY_NAME, "ab" );
      
      if ( VPtCont->PfDestCpy == NULL ) {
        err_warning (( ERR_OUT, "Destination file %s open for append failed => %s", FIL__DUT_INDEX_FILE_CPY_NAME, _strerror ( "System says :" ) ));
        break;
      }
                
      VRet = fwrite ( VAFields, 32 /* size = 8 x U32 */, 1 /* Elt nb */, VPtCont->PfDestCpy );
      
      if ( VRet != 1 ) {
        err_warning (( ERR_OUT, "Error writing record %d => %s", EvId, _strerror ( "System says :" ) ));
        break;
      }
      
      VRet = fflush ( VPtCont->PfDestCpy );
      
      if ( VRet != 0 ) {
        err_warning (( ERR_OUT, "Error flush record %d => %s", EvId, _strerror ( "System says :" ) ));
        break;
      }
      
      VRet = fclose ( VPtCont->PfDestCpy );
      
      if ( VRet != 0 ) {
        err_warning (( ERR_OUT, "Error fclose record %d => %s", EvId, _strerror ( "System says :" ) ));
        break;
      }
...
Date      : 09/07/2012
Rev       : 13/11/2012
          : - Fill destination buffer with 0 before reading file
Doc date  :
Modif     :
Author    : Gilles CLAUS
E-mail    : claus@lepsi.in2p3.fr
...
SInt32 FIL_FIndexFileDutRead ( char* FilePath, UInt32* PtDest, SInt32 DestSz ) {
  ...
  VFileSz = FIL_FFileSize ( FilePath );
  
  // Check buffer size / file size
  
  if ( DestSz < VFileSz ) {
    err_retfail ( -1, (ERR_OUT,"Buffer size=%d < File size=%d", DestSz, VFileSz ) );
  }
  
  err_error (( ERR_OUT, "TRACE => FileSize=%d", VFileSz ));
  
  // Fill destination buffer with 0
  
  memset ( PtDest, 0, DestSz );
  
  // Open file
  
  VPfDest = fopen ( FilePath, "rb" );
  
  err_retnull ( VPfDest, (ERR_OUT,"Source file %s open failed => %s", FilePath, _strerror ( "System says :" ) ));
  ...
  if ( VRet != 1 ) {
    err_retfail ( -1, (ERR_OUT,"Error reading source file %s => %s", FilePath, _strerror ( "System says :" ) ));
  }

  VRet = fclose ( VPfDest );
  
  if ( VRet != 0 ) {
    err_retfail ( -1, (ERR_OUT,"Error fclose source file %s => %s", FilePath, _strerror ( "System says :" ) ));
  }
...
SInt32 FIL_FIndexFileTelCreate ( char* FilePath ) {
  ...
  char VFuncName[]            = "FIL_FIndexFileTelCreate";
  FIL__TIndexFileTel* VPtCont = &FIL__VGIndexFileTel;
  SInt32 VRet;
...
#ifdef EFRIO__CREATE_INDEX_FILE_CPY

  // Cpy file
  
  VPtCont->PfDestCpy = fopen ( FIL__TEL_INDEX_FILE_CPY_NAME, "wb" );
  
  err_retnull ( VPtCont->PfDestCpy, (ERR_OUT,"Destination file %s creation failed => %s", FIL__TEL_INDEX_FILE_CPY_NAME, _strerror ( "System says :" )) );
...
SInt32 FIL_FIndexFileTelAddEv ( UInt32 EvId, UInt32 AcqId, UInt32 FrId, UInt32 EvTag, UInt16 Trig0, UInt16 Trig1, UInt16 Trig2, UInt16 FrTrigNb, UInt32 SpareU32 ) {
  ...
  char VFuncName[]            = "FIL_FIndexFileTelAddEv";
  FIL__TIndexFileTel* VPtCont = &FIL__VGIndexFileTel;
  UInt32 VAFields[10];
  SInt32 VRet;
  ...
  VRet = fwrite ( VAFields, 40 /* size = 10 x U32 */, 1 /* Elt nb */, VPtCont->PfDest );
  ...
  if ( 0 ) {
    ...
    while (1) {    
      ...
      VPtCont->PfDestCpy = fopen ( FIL__TEL_INDEX_FILE_CPY_NAME, "ab" );
      
      if ( VPtCont->PfDestCpy == NULL ) {
        err_warning (( ERR_OUT, "Destination file %s open for append failed => %s", FIL__TEL_INDEX_FILE_CPY_NAME, _strerror ( "System says :" ) ));
        break;
      }
              
      VRet = fwrite ( VAFields, 40 /* size = 10 x U32 */, 1 /* Elt nb */, VPtCont->PfDestCpy );
...
SInt32 FIL_FIndexFileTelRead ( char* FilePath, UInt32* PtDest, SInt32 DestSz ) {
  ...
  char VFuncName[] = "FIL_FIndexFileTelRead";
...
SInt32 FIL_FIndexFileSyncWrite ( char* FilePath, UInt8* PtSrc, SInt32 SrcSz ) {
  
  char VFuncName[]            = "FIL_FIndexFileSyncWrite";
  SInt32 VRet;
  FILE*  VPf;
  ...
  VPf = fopen ( FilePath, "wb" );
  
  err_retnull ( VPf, (ERR_OUT,"Sync index file %s creation failed => %s", FilePath, _strerror ( "System says :" )) );
  
  
  // Try to write
  
  VRet = fwrite ( PtSrc, SrcSz, 1 /* Elt nb */, VPf );
  
  
  if ( VRet != 1 ) {
    err_retfail ( -1, (ERR_OUT,"Error writing sync file %s => %s", FilePath, _strerror ( "System says :" ) ));
  }
  
  VRet = fflush ( VPf );
  
  if ( VRet != 0 ) {
    err_retfail ( -1, (ERR_OUT,"Error flush sync file %s => %s", FilePath, _strerror ( "System says :" ) ));
  }
  
  VRet = fclose ( VPf );
  
  if ( VRet != 0 ) {
    err_retfail ( -1, (ERR_OUT,"Error fclose sync file %s => %s", FilePath, _strerror ( "System says :" ) ));
  }
...
SInt32 FIL_FIndexFileSyncRead ( char* FilePath, UInt8* PtDest, SInt32 DestSz ) {
  
  char VFuncName[]            = "FIL_FIndexFileSyncRead";
  SInt32 VRet;
  FILE*  VPfDest;
  SInt32 VFileSz;
...
SInt32 FIL_FMi32aBTIneffEvFileRead ( char* SrcFilePath, FIL__TMi32ABTIneffEvRec* PtDest, SInt32 DestEltSz ) {
  ...
  char   VStrCR[2];
  char   VDataSep;
  SInt8  VEndOfFileReached;
  
  

  /* Check parameters */
  
  err_retnull ( SrcFilePath, (ERR_OUT,"SrcFilePath == NULL" ) );
  err_retnull ( PtDest     , (ERR_OUT,"PtDest      == NULL" ) );
  
   
  /* Try to open file */
  
  VPf = fopen ( SrcFilePath, "rt" );
  err_retnull ( VPf, (ERR_OUT,"Open source file %s failed", SrcFilePath ) );
  ...
  for ( ViLine=0; ViLine < DestEltSz; ViLine++ ) {
          
    VRet = fscanf ( VPf, "%d%c%d\n", &PtDest[ViLine].EventNo, &VDataSep, &PtDest[ViLine].TrigLine );
      
    if ( VRet == EOF ) {
      VEndOfFileReached = 1;
      fclose (VPf);
      err_trace (( ERR_OUT, "End of file reached : %d lines = records read", ViLine + 1 ));
      break;
    }
    ...
    if ( VRet != 3 ) {
      fclose (VPf);
      break;
      // err_retfail ( -1, (ERR_OUT,"Error reading line=%d in file=%s VRet=%d => %s => Abort", ViLine, SrcFilePath, VRet, _strerror ( "System says :" ) ) );
    }
    
    if ( VDataSep != '-' ) {
      err_warning (( ERR_OUT, "Warning reading line=%d in file=%s => Bad separator = %c", ViLine, VDataSep ));
    }
  ...
  if ( VEndOfFileReached = 0 ) {
    err_error (( ERR_OUT, "Reading of file %s stopped after %d lines = records because max buffer size reached", SrcFilePath, ViLine ));
  }

  fclose (VPf);
    

  // Return nb of lines = records
...
SInt32 FIL_FMi32aBTIneffEvPrint ( FIL__TMi32ABTIneffEvRec* PtSrc, SInt32 RecordsNb ) {
  
  char VFuncName[]            = "FIL_FMi32ABTIneffEvPrint";
  SInt32 VRet;
  SInt32 ViRec;
  
    
  msg (( MSG_OUT, "=====================================" ));
  msg (( MSG_OUT, "Print list of ineff events : %d record", RecordsNb ));
  msg (( MSG_OUT, "=====================================" ));
  msg (( MSG_OUT, "" ));
  


  for ( ViRec=0; ViRec < RecordsNb; ViRec++ ) {
    msg (( MSG_OUT, "Record[%.4d] : Event=%.6d - TrigLine=%.3d", ViRec, PtSrc[ViRec].EventNo, PtSrc[ViRec].TrigLine ));
  }
