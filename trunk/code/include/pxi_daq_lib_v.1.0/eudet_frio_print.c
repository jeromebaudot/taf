    ...
    if ( VDataW16Length != 0 ) {
      ...
      while ( ViSrcW16 < VLastW16 ) { // Odd W16 nb handling => Don't process last W16
        ...
        for ( ViState=0; ViState < VStatesNbPerLine; ViState++ ) {
          ...
          sprintf ( VStrState, "[Col %4d - %2d pixel(s)] ", VState.F.ColAddr, VState.F.HitNb + 1 );
...
SInt32 EFRIO__FPrintFrameRec ( EFRIO__TFrame* PtRec, SInt8 PrintLevel ) {
  ...
  if ( PrintLevel == 1 ) {
    msg (( MSG_OUT, "==============================================" ));
    msg (( MSG_OUT, "AcqId = %.4d - FrameIdInAcq = %.4d - TrigNb = %.4d - Address trig %d [D]", VPtHead->AcqId, VPtHead->FrameIdInAcq, VPtTrig->TrigNb, VPtTrig ));
  ...
  msg (( MSG_OUT, "----------------------------------------------" ));
  msg (( MSG_OUT, "H.TriggerNb    = %.4d  - Address trig %d [D]", VPtHead->TriggerNb, VPtTrig ));
